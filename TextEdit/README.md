...
The primary integration is now `TextEdit::FlexUI`, exposed by
`FlexUITextEditor.h`. It uses flexUI for text input, IME, selection, caret,
scrolling, layout sizing, and themed rendering through `ComputedStyle` and
`RenderCommandList`.

```cmake
target_link_libraries(app PRIVATE TextEdit::FlexUI)
```

```cpp
#include <FlexUITextEditor.h>

auto* element = box.create_widget<textedit::TextEditor>(
    "code-editor", "source", "int main() {}\n");
```

Theme the editor using the existing flexUI textarea variables:
...
#source {
  ...
  --textarea-selection-bg: rgba(37, 99, 235, 0.65);
  --textarea-cursor: #f8fafc;
}
```

`TextEditor.h` and `TextEditor.cpp` contain the legacy ImGui implementation.
They remain in the repository for source compatibility but are not part of the
`TextEdit::FlexUI` target.

## Legacy ImGui implementation
