...
*/
  ...
  #include <nanogui/opengl.h> // Must be included first (includes GLAD)
...
static DitherMatrix dither_matrix(float scale) {
  ...
  DitherMatrix mat = {{0,  32, 8,  40, 2,  34, 10, 42, 48, 16, 56, 24, 50, 18, 58, 26,
...
ColorPass::ColorPass(Texture *color_texture, Texture *depth_texture, Texture *stencil_texture,
                     uint32_t bits_per_channel, bool float_buffer)
  ...
  const float dither_scale = m_float_buffer ? 0.f : (1.f / (1u << bits_per_channel));
...
ColorPass::~ColorPass() {}
  ...
  #if defined(NANOGUI_BUILD_GLFW)
void ColorPass::configure(GLFWwindow *window, float display_sdr_white_level_override) {
  ...
  display_sdr_white_level = display_sdr_white_level_override > 0
                                ? display_sdr_white_level_override
                                : glfwGetWindowSdrWhiteLevel(window);
  display_transfer_function = glfwGetWindowTransfer(window);
  display_primaries = glfwGetWindowPrimaries(window);
    #else
  if (display_sdr_white_level_override > 0)
    display_sdr_white_level = display_sdr_white_level_override;
    #endif

  m_color_shader->set_uniform("display_sdr_white_level", display_sdr_white_level);
  m_color_shader->set_uniform("out_transfer_function", (int)display_transfer_function);

  const auto display_chroma = chroma_from_wp_primaries(display_primaries);
  ...
  m_color_shader->set_uniform("max_luminance", glfwGetWindowMaxLuminance(window));
    #else
  m_color_shader->set_uniform("min_luminance", 0.0f);
  m_color_shader->set_uniform("max_luminance", 80.0f);
    #endif

  m_color_shader->set_uniform("clip_to_unit_interval", !m_float_buffer);

  m_color_shader->set_uniform("dither_scale", Vector2f(tex->size()) * (1.f / DITHER_MATRIX_SIZE));
