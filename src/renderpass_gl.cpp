...
RenderPass::RenderPass(const std::vector<Object *> &color_targets,
    ...
    : m_targets(color_targets.size() + 2),
      ...
      m_framebuffer_handle(0), m_owns_framebuffer(true) {
...
RenderPass::~RenderPass() {
    ...
    if (m_owns_framebuffer && m_framebuffer_handle)
        CHK(glDeleteFramebuffers(1, &m_framebuffer_handle));
}
...
void RenderPass::blit_to(const Vector2i &src_offset,
    ...
    GLint prev_draw_framebuffer = 0;
    CHK(glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING, &prev_read_framebuffer));
    CHK(glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, &prev_draw_framebuffer));
    ...
    CHK(glBindFramebuffer(GL_READ_FRAMEBUFFER, prev_read_framebuffer));
    CHK(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, prev_draw_framebuffer));
#endif
}
