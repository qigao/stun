...
static int sdl_button_to_nanogui(Uint8 button) {
    switch (button) {
        case SDL_BUTTON_LEFT:   return NANOGUI_MOUSE_BUTTON_LEFT;
        case SDL_BUTTON_RIGHT:  return NANOGUI_MOUSE_BUTTON_RIGHT;
        case SDL_BUTTON_MIDDLE: return NANOGUI_MOUSE_BUTTON_MIDDLE;
...
static int sdl_mods_to_nanogui(SDL_Keymod mods) {
    int result = 0;
    if (mods & SDL_KMOD_SHIFT) result |= GLFW_MOD_SHIFT;
    if (mods & SDL_KMOD_CTRL)  result |= GLFW_MOD_CONTROL;
    if (mods & SDL_KMOD_ALT)   result |= GLFW_MOD_ALT;
    if (mods & SDL_KMOD_GUI)   result |= GLFW_MOD_SUPER;
    return result;
}
...
static int sdl_key_to_glfw(SDL_Keycode key) {
    ...
    if (key >= SDLK_SPACE && key <= SDLK_AT) return key;
    ...
    switch (key) {
        ...
        case SDLK_CAPSLOCK: return GLFW_KEY_CAPS_LOCK;
        case SDLK_SCROLLLOCK: return GLFW_KEY_SCROLL_LOCK;
        case SDLK_NUMLOCKCLEAR: return GLFW_KEY_NUM_LOCK;
        case SDLK_PRINTSCREEN: return GLFW_KEY_PRINT_SCREEN;
        case SDLK_PAUSE: return GLFW_KEY_PAUSE;
        default: return GLFW_KEY_UNKNOWN;
    }
}

Screen::Screen()
    : Widget(nullptr), m_sdl_window(nullptr), m_sdl_context(nullptr),
      m_nvg_context(nullptr), m_cursor(Cursor::Arrow),
      m_background(0.3f, 0.3f, 0.32f, 1.f), m_fullscreen(false),
      m_depth_buffer(false), m_stencil_buffer(false), m_float_buffer(false),
      m_redraw(false), m_running(true), m_last_run_mode(RunMode::Stopped),
      m_frame_index(0) {
...
Screen::Screen(const Vector2i &size, std::string_view caption, bool resizable,
    ...
    : Widget(nullptr), m_sdl_window(nullptr), m_sdl_context(nullptr),
      m_nvg_context(nullptr), m_cursor(Cursor::Arrow),
      m_background(0.3f, 0.3f, 0.32f, 1.f), m_caption(caption),
      m_fullscreen(fullscreen), m_depth_buffer(depth_buffer),
      m_stencil_buffer(stencil_buffer), m_float_buffer(float_buffer),
      m_redraw(false), m_running(true), m_last_run_mode(RunMode::Stopped),
      m_frame_index(0) {
    memset(m_cursors, 0, sizeof(SDL_Cursor *) * (size_t) Cursor::CursorCount);
...
Screen::~Screen() {
    ...
    auto it = std::find_if(__nanogui_sdl_screens.begin(), __nanogui_sdl_screens.end(),
                          [this](const std::pair<SDL_Window*, Screen*>& p) {
    ...
    // Only quit SDL if this was the last screen
    if (__nanogui_sdl_screens.empty()) {
        SDL_Quit();
    }
}
...
void Screen::handle_sdl_event(const SDL_Event &event) {
    switch (event.type) {
        ...
        case SDL_EVENT_MOUSE_BUTTON_UP: {
            ...
            handle_mouse_button(event.button.button, pressed,
...
void Screen::handle_mouse_motion(float x, float y) {
    ...
    if (!m_drag_active) {
        ...
        if (m_drag_widget) {
            m_drag_widget->mouse_drag_event(
                m_mouse_pos - m_drag_widget->parent()->absolute_position(),
...
void Screen::handle_mouse_button(Uint8 button, bool pressed, float x, float y) {
    ...
    bool btn12 = (nanogui_button == NANOGUI_MOUSE_BUTTON_LEFT || 
                  nanogui_button == NANOGUI_MOUSE_BUTTON_RIGHT);
...
void Screen::handle_mouse_wheel(float x, float y) {
...
// Note: The following methods are implemented in screen.cpp (shared between backends):
// - redraw()
// - nvg_flush()
// - keyboard_event()
