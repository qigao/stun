...
ColorPrimaries from_screen(const Screen *screen) {
    ...
    #else
        return from_wp_primaries(1); // sRGB fallback
    #endif
#elif defined(NANOGUI_USE_SDL3)
    // SDL3 doesn't provide color primaries API yet, use sRGB fallback
    (void)screen;
    return from_wp_primaries(1); // sRGB fallback
#elif defined(NANOGUI_USE_SDL2)
    // SDL2 doesn't provide color primaries API, use sRGB fallback
    (void)screen;
    return from_wp_primaries(1); // sRGB fallback
#else
    (void)screen;
    return from_wp_primaries(1); // Default sRGB fallback
#endif
}
