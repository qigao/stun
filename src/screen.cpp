/*
    src/screen.cpp -- Top-level widget and interface between NanoGUI and windowing backends
...
#if defined(NANOGUI_BUILD_GLFW) || defined(NANOGUI_USE_SDL3)
...
static float get_pixel_ratio(GLFWwindow *window) {
  ...
  #if defined(EMSCRIPTEN)
static EM_BOOL nanogui_emscripten_resize_callback(int eventType, const EmscriptenUiEvent *,
                                                  void *) {
...
// GLFW-specific constructors and initialization
Screen::Screen()
    : Widget(nullptr), m_glfw_window(nullptr), m_nvg_context(nullptr), m_cursor(Cursor::Arrow),
      m_background(0.3f, 0.3f, 0.32f, 1.f), m_shutdown_glfw(false), m_fullscreen(false),
  ...
  CHK(glGetFramebufferAttachmentParameteriv(GL_DRAW_FRAMEBUFFER, GL_DEPTH,
                                            GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE, &n_depth_bits));
  CHK(glGetFramebufferAttachmentParameteriv(
      GL_DRAW_FRAMEBUFFER, GL_STENCIL, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE, &n_stencil_bits));
...
Screen::Screen(const Vector2i &size, std::string_view caption, bool resizable, bool maximized,
    ...
    : Widget(nullptr), m_glfw_window(nullptr), m_nvg_context(nullptr), m_cursor(Cursor::Arrow),
      m_background(0.3f, 0.3f, 0.32f, 1.f), m_caption(caption), m_shutdown_glfw(false),
      m_fullscreen(fullscreen), m_depth_buffer(depth_buffer), m_stencil_buffer(stencil_buffer),
      m_float_buffer(float_buffer), m_redraw(false), m_last_run_mode(RunMode::Stopped) {
  ...
  glfwWindowHint(GLFW_DEPTH_BITS, depth_bits);
    ...
    #if (defined(NANOGUI_USE_OPENGL) || defined(NANOGUI_USE_GLES) ||                               \
         defined(NANOGUI_USE_METAL)) &&                                                            \
        defined(GLFW_FLOATBUFFER)
  ...
  for (int i = 0; i < 2; ++i) {
    if (fullscreen) {
      ...
      m_glfw_window =
          glfwCreateWindow(mode->width, mode->height, m_caption.c_str(), monitor, nullptr);
    } else {
      m_glfw_window = glfwCreateWindow(size.x(), size.y(), m_caption.c_str(), nullptr, nullptr);
    }
  ...
  float display_sdr_white_level = 80.0f;
  uint32_t display_transfer_function = 10; // EXT sRGB
  uint32_t display_primaries = 1;          // sRGB

    #if GLFW_VERSION_MAJOR > 3 || (GLFW_VERSION_MAJOR == 3 && GLFW_VERSION_MINOR >= 5)
  display_sdr_white_level = glfwGetWindowSdrWhiteLevel(m_glfw_window);
  display_transfer_function = glfwGetWindowTransfer(m_glfw_window);
  display_primaries = glfwGetWindowPrimaries(m_glfw_window);
    #endif

    #ifdef _WIN32
  // On Windows, it is expensive to query the display SDR level, so we do it once on startup and
  // cache the value.
  ...
  m_wants_color_management =
      display_primaries != 1 || display_transfer_function != 10 || display_sdr_white_level != 80.0f;
  ...
  glfwSetMouseButtonCallback(
      m_glfw_window, [](GLFWwindow *w, int button, int action, int modifiers) {
        for (auto kv : __nanogui_screens) {
          if (w == kv.first) {
...
void Screen::initialize(GLFWwindow *window, bool shutdown_glfw) {
  ...
  if (w != m_size[0] || h != m_size[1]) {
    ...
    emscripten_set_resize_callback(nullptr, nullptr, false, nanogui_emscripten_resize_callback);
  } else if (w != w2 || h != h2) {
  ...
  m_nvg_context = nvgCreateMTL(metal_layer(), metal_command_queue(), flags | NVG_DOUBLE_BUFFER);
    #endif
  ...
  if (m_stencil_buffer || m_depth_buffer) {
    m_depth_stencil_texture = new Texture(
        ...
        Texture::ComponentFormat::UInt32, m_fbsize, Texture::InterpolationMode::Nearest,
        Texture::InterpolationMode::Nearest, Texture::WrapMode::ClampToEdge, 1,
  ...
  if (m_wants_color_management) {
    m_color_texture =
        new Texture(pixel_format(), Texture::ComponentFormat::Float32, m_fbsize,
    ...
    m_color_pass = new ColorPass(
        m_color_texture, m_depth_buffer ? m_depth_stencil_texture : nullptr,
        m_stencil_buffer ? m_depth_stencil_texture : nullptr, bits_per_sample(), m_float_buffer);
  }
    #endif
}

    #if defined(NANOGUI_BUILD_GLFW)
// GLFW-specific destructor and window management
Screen::~Screen() {
  auto it = std::find_if(__nanogui_screens.begin(), __nanogui_screens.end(),
...
void Screen::move_window(const Vector2i &rel) {
  ...
  if (monitor_count > 0) {
    ...
    for (int i = 0; i < monitor_count; ++i) {
      ...
      glfwGetMonitorWorkarea(monitors[i], &monitor_work_pos[0], &monitor_work_pos[1],
                             &monitor_work_size[0], &monitor_work_size[1]);
    ...
    pos = max(min(pos, work_pos + work_size - scaled_size), work_pos);
      ...
      #if defined(_WIN32) || defined(__linux__) || defined(EMSCRIPTEN)
  glfwSetWindowPos(m_glfw_window, pos.x() * m_pixel_ratio, pos.y() * m_pixel_ratio);
      #else
...
void Screen::draw_all() {
  if (run_mode() != RunMode::Lazy || m_redraw) {
    ...
    #endif // NANOGUI_BUILD_GLFW (nested block)
  #endif   // NANOGUI_BUILD_GLFW (main block)

// Shared code for both backends
void Screen::draw_contents() { clear(); }
...
void Screen::draw_tooltip() {
  ...
  nvgTextBounds(m_nvg_context, pos.x(), pos.y(), tooltip.data(), tooltip.data() + tooltip.size(),
                bounds);

  int h = (bounds[2] - bounds[0]) / 2;
  if (h > tooltip_width / 2) {
    nvgTextAlign(m_nvg_context, NVG_ALIGN_CENTER | NVG_ALIGN_TOP);
    nvgTextBoxBounds(m_nvg_context, pos.x(), pos.y(), tooltip_width, tooltip.data(),
                     tooltip.data() + tooltip.size(), bounds);
  ...
  nvgTextBox(m_nvg_context, pos.x() - h, pos.y(), tooltip_width, tooltip.data(),
...
void Screen::redraw() {
  ...
  glfwPostEmptyEvent();
  #endif
  // SDL3: No need to post event, manual event loop will handle it
}

  #if defined(NANOGUI_BUILD_GLFW)
// GLFW-specific callback event handlers
void Screen::cursor_pos_callback_event(double x, double y) {
  ...
  try {
    ...
    if (!m_drag_active) {
      ...
      ret = m_drag_widget->mouse_drag_event(p - m_drag_widget->parent()->absolute_position(),
...
bool Screen::mouse_motion_event_f(const Vector2f &, const Vector2f &, int, int) { return false; }
  ...
  try {
    ...
    if (m_drag_active && action == GLFW_RELEASE && drop_widget != m_drag_widget) {
      m_redraw |= m_drag_widget->mouse_button_event(
          m_mouse_pos - m_drag_widget->parent()->absolute_position(), button, false, m_modifiers);
    }
...
void Screen::resize_callback_event(int, int) {
  ...
  #endif // NANOGUI_BUILD_GLFW (callback methods)

// Shared widget management methods
...
bool Screen::maximize_event(bool /*maximized*/) { return false; }

bool Screen::mouse_motion_event_f(const Vector2f &p, const Vector2f &rel, int button,
                                  int modifiers) {
  return mouse_motion_event(Vector2i((int)p.x(), (int)p.y()), Vector2i((int)rel.x(), (int)rel.y()),
                            button, modifiers);
}
...
uint32_t Screen::framebuffer_handle() const {
  ...
  return applies_color_management() ? m_color_pass->framebuffer_handle() : 0;
    #else
  return 0; // SDL3: use default framebuffer
    #endif
}
  #endif
...
Texture::ComponentFormat Screen::component_format() const {
  ...
  #if defined(NANOGUI_USE_METAL)
void *Screen::metal_layer() const { return metal_window_layer(m_nswin); }
  #endif

NAMESPACE_END(nanogui)

#endif // NANOGUI_BUILD_GLFW || NANOGUI_USE_SDL3
