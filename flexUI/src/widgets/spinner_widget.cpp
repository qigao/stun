/*
 * flexUI - SpinnerWidget Implementation
 */
...
void SpinnerWidget::render_ring(RenderCommandList& commands, const Element& elem) {
  ...
  for (int i = 0; i <= segments; i++) {
    ...
    if (i == 0) {
      offset += stbsp_snprintf(path + offset, sizeof(path) - offset, "M %.4g %.4g", x, y);
    } else {
      offset += stbsp_snprintf(path + offset, sizeof(path) - offset, " L %.4g %.4g", x, y);
    }
  }
