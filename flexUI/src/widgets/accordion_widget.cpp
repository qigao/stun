...
void AccordionWidget::add_section(const std::string& title, const std::string& id, float content_height) {
  ...
  set_host_attribute("data-orientation", "vertical");
  set_host_attribute("aria-orientation", "vertical");
  set_host_attribute("data-state", "closed");
  set_host_boolean_attribute("aria-expanded", false);
  set_host_attribute("data-expanded-count", "0");
  clear_host_attribute("data-expanded-id");
}
  ...
  for (size_t i = 0; i < sections_.size(); i++) {
    if (sections_[i].id == id) {
      ...
      if (sections_.empty()) {
        ...
        set_host_attribute("data-state", expanded_count > 0 ? "open" : "closed");
      }
      set_host_boolean_attribute("aria-expanded", expanded_count > 0);
      set_host_attribute("data-expanded-count", std::to_string(expanded_count));
      if (expanded_count == 1 && !expanded_id.empty()) {
        set_host_attribute("data-expanded-id", expanded_id);
      } else {
        clear_host_attribute("data-expanded-id");
      }
      return;
...
void AccordionWidget::emit_render_commands(const Element& elem, RenderCommandList& commands) {
  size_t expanded_count = 0;
  std::string expanded_id;
  for (const auto& section : sections_) {
    if (!section.expanded) {
  ...
  for (size_t i = 0; i < sections_.size(); i++) {
    render_section(commands, elem, sections_[i], y, i);
    y += header_height;
...
void AccordionWidget::render_section(RenderCommandList& commands, const Element& elem, Section& section, float y, size_t idx) {
  ...
  commands.draw_rect(0, y, elem.width(), header_height, 0,
  ...
  commands.stroke_path(path, Paint::solid(border_color), 1);

  render_arrow(commands, 16, y + header_height / 2, section.expanded);

  if (style) {
    const auto text_block = layout_text_block(
        style, section.title, 40.0f, y,
        std::max(0.0f, elem.width() - 52.0f), header_height, text_color,
  ...
  if (progress > 0) {
    ...
    commands.draw_rect(0, content_y, elem.width(), visible_height, 0,
                       Paint::solid(Color{0.98f, 0.98f, 0.98f, 1.0f}),
                       Paint::none(), 0);
  }
}

void AccordionWidget::render_arrow(RenderCommandList& commands, float x, float y, bool expanded) {
  char path[128];
