...
bool load_demo_fonts() {
    bool ok = false;
    ok |= flex::opengl_backend::load_font("Arial", "C:/Windows/Fonts/arial.ttf");
    ok |= flex::opengl_backend::load_font("Consolas", "C:/Windows/Fonts/consola.ttf");
    ok |= flex::opengl_backend::load_font("Segoe UI", "C:/Windows/Fonts/segoeui.ttf");
    return ok;
}
...
public:
    bool init() {
        ...
        window_ = glfwCreateWindow(1100, 760, "flexUI OpenGL Demo", nullptr, nullptr);
        if (!window_) {
        ...
        opengl_canvas_.get_proc_address = [](void*, const char* name) {
            return reinterpret_cast<flex::opengl_backend::OpenGLProcAddress>(
                glfwGetProcAddress(name));
        };
        renderer_ = flex::opengl_backend::create_renderer(&opengl_canvas_);
        if (!renderer_) {
            std::cerr << "OpenGL renderer creation failed\n";
            return false;
        }
...
private:
    void build_ui() {
        ...
        auto* title = box_->create_widget<flexUI::LabelWidget>("div", "title", "flexUI on OpenGL");
        hero->append(title);

        auto* subtitle = box_->create_widget<flexUI::LabelWidget>(
            "div", "subtitle", "Minimal GLFW + OpenGL host with flexUI events, input and progress.");
        hero->append(subtitle);

        const auto caps = box_->renderer_capabilities();
        auto* backend_note = box_->create_widget<flexUI::LabelWidget>(
            "div", "backend-note",
            std::string("OpenGL ") + flexui_examples::renderer_capability_label(caps));
        ...
        static_cast<flexUI::TextAreaWidget*>(editor->widget)->set_placeholder("Textarea on OpenGL...");
        root->append(editor);
    ...
    static LRESULT CALLBACK hook_wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
        auto* app = reinterpret_cast<OpenGLFlexUIDemo*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    ...
    static void mouse_button_callback(GLFWwindow* window, int button, int action, int) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_mouse_button(button, action);
    }

    static void cursor_pos_callback(GLFWwindow* window, double x, double y) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_cursor_pos(x, y);
    }

    static void scroll_callback(GLFWwindow* window, double dx, double dy) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_scroll(dx, dy);
    }

    static void key_callback(GLFWwindow* window, int key, int, int action, int mods) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_key(key, action, mods);
    }

    static void char_callback(GLFWwindow* window, unsigned int codepoint) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_char(codepoint);
    }

    static void framebuffer_size_callback(GLFWwindow* window, int width, int height) {
        static_cast<OpenGLFlexUIDemo*>(glfwGetWindowUserPointer(window))->on_resize(width, height);
    }

    GLFWwindow* window_ = nullptr;
    flex::opengl_backend::OpenGLCanvas opengl_canvas_;
