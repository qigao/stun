...
private:
  ...
  void renderSvgGauge(NVGcontext *ctx, tvg::Picture *svg, float cx, float cy, float size);
  void drawGaugeTicks(NVGcontext *ctx, float cx, float cy, float radius, 
                      float minVal, float maxVal, const char *unit);

  std::unique_ptr<tvg::Picture> m_gaugeSvg;
  std::unique_ptr<tvg::Picture> m_needleSvg;
  float m_speed;
  float m_targetSpeed;
  float m_rpm;
  float m_targetRpm;
