...
StylePanelModule::StylePanelModule(Widget *parent) : Widget(parent) {
  ...
  for (size_t i = 0; i < bgCols.size(); i++) {
    float x = padding + i * (buttonSize + spacing);
    bool hasPattern = (i == 5); // Last one has checkerboard pattern
  ...
  for (int i = 0; i < 3; i++) {
    float x = padding + i * (buttonSize + spacing);
    m_arrowTypeButtons.push_back({x, y, buttonSize, "arrow", i == 2, i});
  }
  ...
  for (int i = 0; i < 2; i++) {
    float x = padding + i * (buttonSize + spacing);
    m_endpointButtons.push_back({x, y, buttonSize, "endpoint", false, i});
  }

  // Opacity slider will be at y + 100
  // Layer controls will be at y + 200
...
Vector2i StylePanelModule::preferred_size_impl(NVGcontext *) const {
  return {370, 1100}; // Vertical panel
}

bool StylePanelModule::mouse_button_event(const Vector2i &p, int button, bool down, int modifiers) {
  if (button == GLFW_MOUSE_BUTTON_1 && down) {
    ...
    // Check arrow type buttons
    for (auto &btn : m_arrowTypeButtons) {
      float dx = localPos.x() - (btn.x + btn.size * 0.5f);
      float dy = localPos.y() - (btn.y + btn.size * 0.5f);
      if (std::abs(dx) <= btn.size * 0.5f && std::abs(dy) <= btn.size * 0.5f) {
        for (auto &b : m_arrowTypeButtons)
          b.selected = false;
        btn.selected = true;
        screen()->redraw();
        return true;
    ...
    // Check endpoint buttons
    for (auto &btn : m_endpointButtons) {
      float dx = localPos.x() - (btn.x + btn.size * 0.5f);
      float dy = localPos.y() - (btn.y + btn.size * 0.5f);
      if (std::abs(dx) <= btn.size * 0.5f && std::abs(dy) <= btn.size * 0.5f) {
        for (auto &b : m_endpointButtons)
          b.selected = false;
        btn.selected = true;
        screen()->redraw();
        return true;
    ...
    // Check layer buttons (action buttons)
    for (auto &btn : m_layerButtons) {
      float dx = localPos.x() - (btn.x + btn.size * 0.5f);
      float dy = localPos.y() - (btn.y + btn.size * 0.5f);
      if (std::abs(dx) <= btn.size * 0.5f && std::abs(dy) <= btn.size * 0.5f) {
        // Layer buttons trigger actions
        screen()->redraw();
        return true;
      }
    }
...
bool StylePanelModule::mouse_drag_event(const Vector2i &p, const Vector2i &rel, int button,
                                        int modifiers) {
  if (m_draggingSlider) {
    Vector2f localPos = Vector2f(p.x() - m_pos.x(), p.y() - m_pos.y());
    float sliderX = 30.f;
...
void StylePanelModule::draw(NVGcontext *ctx) {
  Widget::draw(ctx);
  ...
  // Arrow type section
  drawLabel(ctx, px + 20.f, py + yOffset, "Arrow type");
  yOffset += 30.f;
  for (size_t i = 0; i < m_arrowTypeButtons.size(); i++) {
    const auto &btn = m_arrowTypeButtons[i];
    drawIconButton(ctx, px + btn.x, py + btn.y, btn.size, btn.icon.c_str(), btn.selected);
    drawArrowTypeIcon(ctx, px + btn.x + btn.size * 0.5f, py + btn.y + btn.size * 0.5f,
                      btn.size * 0.4f, static_cast<int>(i));
  }
  yOffset += 80.f;

  // Endpoint section (端点)
  drawLabel(ctx, px + 20.f, py + yOffset, "端点");
  yOffset += 30.f;
  for (size_t i = 0; i < m_endpointButtons.size(); i++) {
    const auto &btn = m_endpointButtons[i];
    drawIconButton(ctx, px + btn.x, py + btn.y, btn.size, btn.icon.c_str(), btn.selected);
    drawEndpointIcon(ctx, px + btn.x + btn.size * 0.5f, py + btn.y + btn.size * 0.5f,
                     btn.size * 0.4f, static_cast<int>(i));
  }
  yOffset += 80.f;
...
void StylePanelModule::drawLabel(NVGcontext *ctx, float x, float y, const char *text) {
...
void StylePanelModule::drawColorButton(NVGcontext *ctx, float x, float y, float size,
  ...
  // Draw checkerboard pattern if needed
  if (hasPattern) {
...
void StylePanelModule::drawIconButton(NVGcontext *ctx, float x, float y, float size,
...
void StylePanelModule::drawSlider(NVGcontext *ctx, float x, float y, float w, float value,
...
void StylePanelModule::drawStrokeWidthIcon(NVGcontext *ctx, float cx, float cy, float size,
                                           int type) {
...
void StylePanelModule::drawBorderStyleIcon(NVGcontext *ctx, float cx, float cy, float size,
                                           int type) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
...
void StylePanelModule::drawLineStyleIcon(NVGcontext *ctx, float cx, float cy, float size,
                                         int type) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
...
void StylePanelModule::drawCornerIcon(NVGcontext *ctx, float cx, float cy, float size,
                                      bool rounded) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
  nvgStrokeWidth(ctx, 2.5f);
...
void StylePanelModule::drawArrowTypeIcon(NVGcontext *ctx, float cx, float cy, float size,
                                         int type) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
  nvgStrokeWidth(ctx, 2.5f);
  nvgLineCap(ctx, NVG_ROUND);
  ...
  if (type == 0) {
    // Simple arrow (straight line with arrow)
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx - size * 0.8f, cy);
    nvgLineTo(ctx, cx + size * 0.8f, cy);
    nvgStroke(ctx);
    // Arrow head
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx + size * 0.5f, cy - size * 0.3f);
    nvgLineTo(ctx, cx + size * 0.8f, cy);
    nvgLineTo(ctx, cx + size * 0.5f, cy + size * 0.3f);
    nvgStroke(ctx);
  } else if (type == 1) {
    // Curved arrow
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx - size * 0.8f, cy + size * 0.3f);
    nvgBezierTo(ctx, cx - size * 0.2f, cy - size * 0.5f, cx + size * 0.2f, cy - size * 0.5f,
                cx + size * 0.8f, cy + size * 0.3f);
    nvgStroke(ctx);
    // Arrow head
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx + size * 0.5f, cy + size * 0.1f);
    nvgLineTo(ctx, cx + size * 0.8f, cy + size * 0.3f);
    nvgLineTo(ctx, cx + size * 0.7f, cy + size * 0.6f);
    nvgStroke(ctx);
  } else {
    // Double curved arrow
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx - size * 0.8f, cy);
    nvgBezierTo(ctx, cx - size * 0.3f, cy - size * 0.6f, cx + size * 0.3f, cy + size * 0.6f,
                cx + size * 0.8f, cy);
    nvgStroke(ctx);
    // Arrow head
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx + size * 0.5f, cy - size * 0.2f);
    nvgLineTo(ctx, cx + size * 0.8f, cy);
    nvgLineTo(ctx, cx + size * 0.6f, cy + size * 0.3f);
    nvgStroke(ctx);
  }
}

void StylePanelModule::drawEndpointIcon(NVGcontext *ctx, float cx, float cy, float size, int type) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
  nvgStrokeWidth(ctx, 2.5f);
  nvgLineCap(ctx, NVG_ROUND);
  ...
  if (type == 0) {
    // X endpoint on left
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx - size * 0.8f, cy - size * 0.2f);
    nvgLineTo(ctx, cx - size * 0.4f, cy + size * 0.2f);
    nvgMoveTo(ctx, cx - size * 0.4f, cy - size * 0.2f);
    nvgLineTo(ctx, cx - size * 0.8f, cy + size * 0.2f);
    nvgStroke(ctx);
  } else {
    // Arrow endpoint on right
    nvgBeginPath(ctx);
    nvgMoveTo(ctx, cx + size * 0.4f, cy - size * 0.2f);
    nvgLineTo(ctx, cx + size * 0.7f, cy);
    nvgLineTo(ctx, cx + size * 0.4f, cy + size * 0.2f);
    nvgStroke(ctx);
  }
}

void StylePanelModule::drawLayerIcon(NVGcontext *ctx, float cx, float cy, float size,
                                     int direction) {
  nvgStrokeColor(ctx, nvgRGBA(60, 60, 80, 255));
  nvgStrokeWidth(ctx, 2.5f);
