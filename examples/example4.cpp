...
public:
  MyCanvas(Widget *parent) : Canvas(parent, 1), m_rotation(0.f) {
    ...
    m_shader = new Shader(render_pass(), "a_simple_shader",
  ...
  virtual void draw_contents() override {
    ...
    Matrix4f proj =
        Matrix4f::perspective(float(25 * Pi / 180), 0.1f, 20.f, m_size.x() / (float)m_size.y());
...
public:
  ExampleApplication() : nanogui::Screen(Vector2i(800, 600), "NanoGUI Test", false) {
    ...
    b0->set_callback([this]() {
      m_canvas->set_background_color(Vector4i(rand() % 256, rand() % 256, rand() % 256, 255));
    });
    Button *b1 = new Button(tools, "Random Rotation");
    b1->set_callback([this]() { m_canvas->set_rotation((float)Pi * rand() / (float)RAND_MAX); });
    perform_layout();
  }
