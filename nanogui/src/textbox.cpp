...
TextBox::TextBox(Widget *parent, std::string_view value)
    : Widget(parent), m_editable(false), m_spinnable(false), m_committed(true), m_value(value),
      m_default_value(""), m_alignment(Alignment::Center), m_units(""), m_format(""),
      m_units_image(-1), m_valid_format(true), m_value_temp(value), m_cursor_pos(-1),
      m_selection_pos(-1), m_mouse_pos(Vector2i(-1, -1)), m_mouse_down_pos(Vector2i(-1, -1)),
      m_mouse_drag_pos(Vector2i(-1, -1)), m_mouse_down_modifier(0), m_corner_radius(2.5f),
      m_text_offset(0), m_last_click(0), m_solid_color(Color(0, 0, 0, 0)) {
  if (m_theme)
...
void TextBox::draw(NVGcontext *ctx) {
  ...
  if (m_solid_color.a() > 0) {
    bg = fg1 = fg2 = nvgBoxGradient(ctx, m_pos.x(), m_pos.y(), m_size.x(), m_size.y(), 3, 4,
                                    m_solid_color, m_solid_color);

    nvgRoundedRect(ctx, m_pos.x(), m_pos.y(), m_size.x(), m_size.y(), m_corner_radius);
  } else {
    bg = nvgBoxGradient(ctx, m_pos.x() + 1, m_pos.y() + 1 + 1.0f, m_size.x() - 2, m_size.y() - 2, 3,
                        4, Color(255, 32), Color(32, 32));
    fg1 = nvgBoxGradient(ctx, m_pos.x() + 1, m_pos.y() + 1 + 1.0f, m_size.x() - 2, m_size.y() - 2,
                         3, 4, Color(150, 32), Color(32, 32));
    fg2 = nvgBoxGradient(ctx, m_pos.x() + 1, m_pos.y() + 1 + 1.0f, m_size.x() - 2, m_size.y() - 2,
                         3, 4, nvgRGBA(255, 0, 0, 100), nvgRGBA(255, 0, 0, 50));

    nvgRoundedRect(ctx, m_pos.x() + 1, m_pos.y() + 1 + 1.0f, m_size.x() - 2, m_size.y() - 2,
                   m_corner_radius);
  }
  ...
  nvgRoundedRect(ctx, m_pos.x() + 0.5f, m_pos.y() + 0.5f, m_size.x() - 1, m_size.y() - 1,
                 std::max(m_corner_radius - 0.5f, 0.0f));
  ...
  if (m_units_image > 0) {
    ...
    NVGpaint img_paint = nvgImagePattern(ctx, m_pos.x() + m_size.x() - x_spacing - unit_width,
                                         draw_pos.y() - unit_height * 0.5f, unit_width, unit_height,
                                         0, m_units_image, m_enabled ? 0.7f : 0.35f);
    nvgBeginPath(ctx);
    nvgRect(ctx, m_pos.x() + m_size.x() - x_spacing - unit_width, draw_pos.y() - unit_height * 0.5f,
  ...
  if (m_spinnable && !focused()) {
    ...
    nvgFontSize(ctx,
                ((m_font_size < 0) ? m_theme->m_button_font_size : m_font_size) * icon_scale());

    bool spinning = m_mouse_down_pos.x() != -1;

    /* up button */ {
      bool hover = m_mouse_focus && spin_area(m_mouse_pos) == SpinArea::Top;
      nvgFillColor(ctx, (m_enabled && (hover || spinning)) ? m_theme->m_text_color
  ...
  if (m_committed) {
    ...
    int nglyphs = nvgTextGlyphPositions(ctx, draw_pos.x(), draw_pos.y(), m_value_temp.c_str(),
    ...
    // recompute cursor positions first
    nglyphs = nvgTextGlyphPositions(ctx, draw_pos.x(), draw_pos.y(), m_value_temp.c_str(), nullptr,
                                    glyphs, max_glyphs);
    ...
    if (m_cursor_pos > -1) {
      if (m_selection_pos > -1) {
        ...
        // Draw selection background with rounded corners BEFORE text
        nvgBeginPath(ctx);
        nvgFillColor(ctx, nvgRGBA(100, 150, 255, 60));
        float radius = lineh * 0.25f;
        nvgRoundedRect(ctx, caretx - 2, draw_pos.y() - lineh * 0.5f - 2, 
                      selx - caretx + 4, lineh + 4, radius);
        nvgFill(ctx);
        
        // Draw subtle outline around selection area
        nvgBeginPath(ctx);
        nvgRoundedRect(ctx, caretx - 2, draw_pos.y() - lineh * 0.5f - 2, 
                      selx - caretx + 4, lineh + 4, radius);
        nvgStrokeColor(ctx, nvgRGBA(100, 150, 255, 180));
        nvgStrokeWidth(ctx, 1.5f);
        nvgStroke(ctx);
      }
    }
    
    // Draw text AFTER selection background
...
bool TextBox::mouse_button_event(const Vector2i &p, int button, bool down, int modifiers) {
  ...
  if (m_editable && focused()) {
    if (down) {
      ...
      double time = Screen::get_time();
      if (time - m_last_click < 0.25) {
        /* Double-click: select all text */
        m_selection_pos = 0;
        m_cursor_pos = utf8_char_count(m_value_temp);
...
bool TextBox::mouse_motion_event(const Vector2i &p, const Vector2i & /* rel */, int /* button */,
  ...
  if (!m_editable)
    set_cursor(Cursor::Arrow);
           ...
           spin_area(m_mouse_pos) != SpinArea::None) /* scrolling arrows */
  ...
  if (m_editable && focused()) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
      if (key == GLFW_KEY_ESCAPE) {
        ...
        if (m_cursor_pos > 0)
          m_cursor_pos--;  // Move by 1 character
        ...
        // Move by 1 character (not byte)
        int char_count = utf8_char_count(m_value_temp);
        if (m_cursor_pos < char_count)
          m_cursor_pos++;
        ...
        if (!delete_selection()) {
          if (m_cursor_pos > 0) {
            ...
            size_t byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos);
            size_t prev_byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos - 1);
            m_value_temp.erase(prev_byte_pos, byte_pos - prev_byte_pos);
            m_cursor_pos--;
          }
        }
        ...
        if (!delete_selection()) {
          int char_count = utf8_char_count(m_value_temp);
          if (m_cursor_pos < char_count) {
            // Delete one UTF-8 character at cursor
            size_t byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos);
            size_t next_byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos + 1);
            m_value_temp.erase(byte_pos, next_byte_pos - byte_pos);
          }
        }
...
bool TextBox::keyboard_character_event(unsigned int codepoint) {
  if (m_editable && focused()) {
    // Convert UTF-32 codepoint to UTF-8 string (supports Chinese, emoji, etc.)
    std::string utf8_char = utf8(codepoint);

    delete_selection();
    
    // Convert character position to byte position for insertion
    size_t byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos);
    m_value_temp.insert(byte_pos, utf8_char);
    
    // Move cursor forward by 1 character (not bytes)
    m_cursor_pos++;
...
bool TextBox::check_format(const std::string &input, const std::string &format) {
  ...
  try {
    ...
    std::cerr << "Warning: cannot validate text field due to lacking regular expression support. "
                 "please compile with GCC >= 4.9"
              << std::endl;
    return true;
#else
    throw;
...
bool TextBox::copy_selection() {
  if (m_selection_pos > -1) {
    ...
    size_t byte_begin = char_to_byte_index(m_value_temp, begin);
    size_t byte_end = char_to_byte_index(m_value_temp, end);

#if defined(NANOGUI_BUILD_GLFW)
    glfwSetClipboardString(sc->glfw_window(), m_value_temp.substr(byte_begin, byte_end - byte_begin).c_str());
#elif defined(NANOGUI_USE_SDL2)
    SDL_SetClipboardText(m_value_temp.substr(byte_begin, byte_end - byte_begin).c_str());
#endif
    return true;
  }
...
void TextBox::paste_from_clipboard() {
  ...
  if (cbstr) {
    std::string paste_text(cbstr);
    size_t byte_pos = char_to_byte_index(m_value_temp, m_cursor_pos);
    m_value_temp.insert(byte_pos, paste_text);
    m_cursor_pos += utf8_char_count(paste_text);
  }
#elif defined(NANOGUI_USE_SDL2)
...
bool TextBox::delete_selection() {
  if (m_selection_pos > -1) {
    ...
    m_value_temp.erase(byte_begin, byte_end - byte_begin);
...
TextBox::SpinArea TextBox::spin_area(const Vector2i &pos) {
  if (0 <= pos.x() - m_pos.x() && pos.x() - m_pos.x() < 14.f) { /* on scrolling arrows */
    if (m_size.y() >= pos.y() - m_pos.y() &&
        pos.y() - m_pos.y() <= m_size.y() / 2.f) { /* top part */
      return SpinArea::Top;
    } else if (0.f <= pos.y() - m_pos.y() &&
               pos.y() - m_pos.y() > m_size.y() / 2.f) { /* bottom part */
...
// Helper function: Count UTF-8 characters in a string
int TextBox::utf8_char_count(const std::string& str) {
  return static_cast<int>(utf8len(reinterpret_cast<const utf8_int8_t*>(str.c_str())));
}

// Helper function: Convert character index to byte index
size_t TextBox::char_to_byte_index(const std::string& str, int char_index) {
  if (char_index <= 0) return 0;
  
  const utf8_int8_t* s = reinterpret_cast<const utf8_int8_t*>(str.c_str());
  const utf8_int8_t* start = s;
  int count = 0;
  utf8_int32_t codepoint;
  
  while (*s && count < char_index) {
    s = utf8codepoint(s, &codepoint);
    count++;
  }
