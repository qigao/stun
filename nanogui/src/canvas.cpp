...
Canvas::Canvas(Widget *parent, uint8_t samples, bool has_depth_buffer, bool has_stencil_buffer,
  ...
  if (!m_render_to_texture) {
    ...
    color_texture =
        new Texture(scr->pixel_format(), scr->component_format(), m_size,
    ...
    depth_texture = new Texture(
        has_stencil_buffer ? Texture::PixelFormat::DepthStencil : Texture::PixelFormat::Depth,
  ...
  m_render_pass =
      new RenderPass({color_texture}, depth_texture, has_stencil_buffer ? depth_texture : nullptr,
#if defined(NANOGUI_USE_METAL)
...
const Color &Canvas::background_color() const { return m_render_pass->clear_color(0); }
