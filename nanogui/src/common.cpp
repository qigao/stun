...
extern std::vector<std::pair<SDL_Window *, Screen *>> __nanogui_sdl_screens;
#endif
...
void init(bool color_management) {
  ...
  glfwSetTime(0);
#elif defined(NANOGUI_USE_SDL3)
  // SDL3 initialization is handled per-Screen in screen_sdl.cpp
...
static void mainloop_iteration_sdl() {
  int num_screens = 0;
  ...
  for (auto kv : __nanogui_sdl_screens) {
    Screen *screen = kv.second;
    if (!screen->visible()) {
      continue;
    }

    // Process events for this screen
...
void shutdown() {
  ...
  glfwTerminate();
#elif defined(NANOGUI_USE_SDL3)
  // SDL3 cleanup - only quit if all screens are destroyed
  extern std::vector<std::pair<SDL_Window *, Screen *>> __nanogui_sdl_screens;
...
file_dialog(Widget *parent, FileDialogType type,
...
#if defined(NANOGUI_BUILD_GLFW)
  NFD_GetNativeWindowFromGLFWWindow(parent->screen()->glfw_window(), &parent_window);
#endif
  ...
  case FileDialogType::PickFolderMultiple: {
    ...
    if (!filters.empty())
      throw std::runtime_error(
          "nanogui::file_dialog(): filters are not supported for folder selection.");
  ...
  if (result == NFD_CANCEL)
    ...
    throw std::runtime_error("nanogui::file_dialog(): dialog error: " +
  ...
  case FileDialogType::OpenMultiple:
    if (!NFD_PathSet_GetEnum(out_paths, &enumerator))
      throw std::runtime_error("nanogui::file_dialog(): could not obtain enumerator: " +
                               std::string(NFD_GetError()));
