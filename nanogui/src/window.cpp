...
bool Window::mouse_drag_event(const Vector2i &, const Vector2i &rel,
    ...
    if (m_drag && (button & (1 << NANOGUI_MOUSE_BUTTON_LEFT)) != 0) {
        m_pos += rel;
        Vector2i parent_size = parent() ? parent()->size() : Vector2i(0);
        for (int i = 0; i < 2; ++i) {
            int delta = parent_size[i] - m_size[i];
            int min_bound = std::min(0, delta);
            int max_bound;
            if (delta >= 0) {
                max_bound = delta;
            } else {
                if (i == 1) {
                    int header = m_theme ? m_theme->m_window_header_height : 0;
                    int header_limit = parent_size[i] - header;
                    max_bound = std::max(delta, header_limit);
                } else {
                    max_bound = 0;
                }
            }
            m_pos[i] = std::max(m_pos[i], min_bound);
            m_pos[i] = std::min(m_pos[i], max_bound);
        }
        return true;
    }
    return false;
