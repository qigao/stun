...
static_assert(std::is_same_v<decltype(gcanvas::Window::create(
                                 std::declval<gcanvas::WindowConfig>())),
...
int main()
{
    ...
    if (limits.max_images != 256 || limits.max_fonts != 32 ||
        limits.max_path_surfaces != 16 ||
        limits.max_path_surface_pixels != 4096U * 4096U ||
        limits.max_path_mesh_quads != 262144U || limits.path_paint_texture_size != 256U ||
        limits.max_clip_vertices != 64U || limits.max_shadow_samples != 25U ||
        limits.max_blur_samples != 25U || limits.max_blur_commands != 65536U)
    {
        return 3;
    }
    const gcanvas::Path path = gcanvas::Path::from_svg(
        "M1 2 L3 4 h5 v6 q1 2 3 4 c1 2 3 4 5 6 a4 3 30 0 1 10 8 z");
    if (path.empty() || path.commands().size() < 8U)
    {
        return 4;
    }
    const gcanvas::Transform transform =
        gcanvas::Transform::translation(4.0f, 5.0f) *
    ...
    if (std::fabs(restored_x - 7.0f) > 0.001f ||
        std::fabs(restored_y - 9.0f) > 0.001f)
    {
        return 6;
    }
    ...
    if (gradient.stops().front().offset != 0.0f ||
        gradient.stops().back().offset != 1.0f)
    {
        return 7;
    }
    return 0;
