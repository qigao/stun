...
static void vec_scale(float* x, int n, float alpha)
{
    int          i = 0;
    simde__m128 a = simde_mm_set1_ps(alpha);

    for (; i + 4 <= n; i += 4)
    {
        simde__m128 xv = simde_mm_loadu_ps(x + i);
        simde_mm_storeu_ps(x + i, simde_mm_mul_ps(xv, a));
    }
    for (; i < n; ++i)
    {
        x[i] *= alpha;
...
static void vec_axpy(float* y, const float* x, int n, float alpha)
{
    int          i = 0;
    simde__m128 a = simde_mm_set1_ps(alpha);
    ...
    for (; i + 4 <= n; i += 4)
    {
        simde__m128 yv = simde_mm_loadu_ps(y + i);
        simde__m128 xv = simde_mm_loadu_ps(x + i);
        simde_mm_storeu_ps(y + i, simde_mm_add_ps(yv, simde_mm_mul_ps(a, xv)));
    }
    for (; i < n; ++i)
    {
        y[i] += alpha * x[i];
...
static void vec_axpby(float* y, const float* x, int n, float alpha, float beta)
{
    int          i = 0;
    simde__m128 a = simde_mm_set1_ps(alpha);
    simde__m128 b = simde_mm_set1_ps(beta);
    ...
    if (beta == 0.f)
    {
        ...
        {
            simde__m128 xv = simde_mm_loadu_ps(x + i);
            simde_mm_storeu_ps(y + i, simde_mm_mul_ps(a, xv));
        }
        for (; i < n; ++i)
        {
            y[i] = alpha * x[i];
    ...
    for (; i + 4 <= n; i += 4)
    {
        simde__m128 yv = simde_mm_loadu_ps(y + i);
        simde__m128 xv = simde_mm_loadu_ps(x + i);
        simde_mm_storeu_ps(y + i, simde_mm_add_ps(simde_mm_mul_ps(b, yv), simde_mm_mul_ps(a, xv)));
    }
    for (; i < n; ++i)
    {
...
static float vec_dot(const float* x, const float* y, int n)
{
    int          i   = 0;
    float        sum = 0.f;
    float        lane[4];
    simde__m128 acc = simde_mm_setzero_ps();

    for (; i + 4 <= n; i += 4)
    {
        simde__m128 xv = simde_mm_loadu_ps(x + i);
        simde__m128 yv = simde_mm_loadu_ps(y + i);
        acc            = simde_mm_add_ps(acc, simde_mm_mul_ps(xv, yv));
    }

    simde_mm_storeu_ps(lane, acc);
          ...
          float* alpha, const float* a, int* lda, float* b, int* ldb)
{
    ...
    else if (*lda < mb_max(1, nrowa))
    {
        info = 9;
    }
    else if (*ldb < mb_max(1, *m))
    {
        info = 11;
    }
    if (info != 0)
          ...
          float* b, int* ldb, float* beta, float* c__, int* ldc)
{
    ...
    else if (*ldb < mb_max(1, nrowb))
    {
        info = 10;
    }
    else if (*ldc < mb_max(1, *m))
    {
        info = 13;
    }
    if (info != 0)
