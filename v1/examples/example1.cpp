/*
    examples/example1.cpp -- C++ version of an example application that shows
...
public:
  ExampleApplication() : Screen(Vector2i(1024, 768), "NanoGUI Test") {
    ...
    b->set_change_callback(
        [](bool state) { std::cout << "Toggle button state: " << state << std::endl; });
    ...
    b->set_callback([&] {
      auto dlg = new MessageDialog(this, MessageDialog::Type::Question, "Title",
    ...
    for (auto &icon : icons) {
      ...
      ImageHolder texture_data(
          stbi_load((icon.second + ".png").c_str(), &size.x(), &size.y(), &n, 0), stbi_image_free);
      assert(n == 4);

      Texture *tex =
          new Texture(Texture::PixelFormat::RGBA, Texture::ComponentFormat::UInt8, size,
    ...
    image_view->set_pixel_callback([this](const Vector2i &index, char **out, size_t size) {
    ...
    text_box->set_units("%");
    slider->set_callback(
        [text_box](float value) { text_box->set_value(std::to_string((int)(value * 100))); });
    ...
    tab_widget->set_callback([tab_widget, this, counter, plus_id](int id) mutable {
      if (id == plus_id) {
        ...
        int new_id = tab_widget->insert_tab(tab_widget->tab_count() - 1, tab_name, layer_dyn);
        ...
        func_dyn.resize(100);
        for (int i = 0; i < 100; ++i)
          func_dyn[i] = 0.5f * std::abs((0.5f * std::sin(i / 10.f + counter) +
    ...
    panel->set_layout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 6));
    ...
    */
    ...
    const float positions[3 * 4] = {-1.f, -1.f, 0.f, 1.f, -1.f, 0.f, 1.f, 1.f, 0.f, -1.f, 1.f, 0.f};
  ...
  virtual bool keyboard_event(int key, int scancode, int action, int modifiers) {
    if (Screen::keyboard_event(key, scancode, action, modifiers))
      return true;
    // ESC key to close (key code 27 for ESC, action 1 for press)
    if (key == 27 && action == 1) {
      set_visible(false);
      return true;
    }
  ...
  virtual void draw(NVGcontext *ctx) {
    ...
    m_progress->set_value(std::fmod((float)Screen::get_time() / 10, 1.0f));
  ...
  virtual void draw_contents() {
    ...
    Matrix4f mvp =
        Matrix4f::scale(Vector3f((float)m_size.y() / (float)m_size.x() * 0.25f, 0.25f, 0.25f)) *
        Matrix4f::rotate(Vector3f(0, 0, 1), time);
