...
TextArea::TextArea(Widget *parent)
    : Widget(parent), m_foreground_color(Color(0, 0)), m_background_color(Color(0, 0)),
      m_selection_color(.5f, 1.f), m_font("sans"), m_offset(0), m_max_size(0), m_padding(0),
      m_selectable(true), m_selection_start(-1), m_selection_end(-1) {}
...
void TextArea::append(std::string_view text) {
  ...
  const char *cur = text.data(), *end = cur + text.length();
...
Vector2i TextArea::preferred_size_impl(NVGcontext *) const { return m_max_size + m_padding * 2; }
...
void TextArea::draw(NVGcontext *ctx) {
  ...
  if (vscroll) {
    ...
    start_it = std::lower_bound(
        m_blocks.begin(), m_blocks.end(), window_offset,
        [&](const Block &block, int value) { return block.offset.y() + font_size() < value; });

    end_it =
        std::upper_bound(m_blocks.begin(), m_blocks.end(), window_offset + window_size,
...
Vector2i TextArea::position_to_block(const Vector2i &pos) const {
  ...
  auto it = std::lower_bound(
      m_blocks.begin(), m_blocks.end(), pos.y(),
      [&](const Block &block, int value) { return block.offset.y() + font_size() < value; });
  ...
  if (it == m_blocks.end()) {
    ...
    it = m_blocks.end() - 1;
    const Block &block = *it;
    selection = nvgTextGlyphPositions(ctx, block.offset.x(), block.offset.y(), block.text.c_str(),
...
Vector2i TextArea::block_to_position(const Vector2i &pos) const {
  ...
  nvgFontSize(ctx, font_size());
  nvgFontFace(ctx, m_font.c_str());
  int nglyphs = nvgTextGlyphPositions(ctx, block.offset.x(), block.offset.y(), block.text.c_str(),
                                      nullptr, glyphs, max_glyphs);
  if (pos.y() == nglyphs)
