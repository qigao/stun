...
Widget::Widget(Widget *parent)
    : m_parent(nullptr), m_theme(nullptr), m_layout(nullptr), m_pos(0),
      m_size(0), m_fixed_size(0), m_visible(true), m_enabled(true),
...
int Widget::font_size() const {
  return (m_font_size < 0 && m_theme) ? m_theme->m_standard_font_size
                                      : m_font_size;
}
    ...
    if (result == Vector2i(-1)) {
      ...
      if (ref != result) {
        fprintf(
            ...
            "size. It appears that something updated its state but did not "
            "call "
            "preferred_size_changed() (size=[%i, %i], ref=[%i, %i])!\n",
            this, typeid(*this).name(), result.x(), result.y(), ref.x(),
            ref.y());
      }
#endif
    }
...
bool Widget::mouse_button_event(const Vector2i &p, int button, bool down,
                                int modifiers) {
  size_t n = m_children.size();
  for (size_t i = n; i > 0; --i) {
    Widget *child = m_children[i-1];
  ...
  if (button == NANOGUI_MOUSE_BUTTON_LEFT && down && !m_focused)
    request_focus();
  return false;
}

bool Widget::mouse_motion_event(const Vector2i &p, const Vector2i &rel,
...
bool Widget::keyboard_event(int, int, int, int) { return false; }

bool Widget::keyboard_character_event(unsigned int) { return false; }
...
void Widget::add_child(Widget *widget) { add_child(child_count(), widget); }
...
void Widget::remove_child_helper(
    const std::vector<Widget *>::iterator &child_it) {
...
void Widget::draw(NVGcontext *ctx) {
  ...
  nvgBeginPath(ctx);
  nvgRect(ctx, m_pos.x() - 0.5f, m_pos.y() - 0.5f, m_size.x() + 1,
          m_size.y() + 1);
  ...
  for (auto child : m_children) {
    ...
    if (has_scissor) {
      Vector2i scissor_min((int)scissor[0], (int)scissor[1]),
          scissor_max =
              scissor_min + Vector2i((int)scissor[2], (int)scissor[3]),
          child_min = child->m_pos, child_max = child_min + child->m_size;

      if (child_min.x() >= scissor_max.x() ||
          child_min.y() >= scissor_max.y() ||
