/*
 * tvgbox2 - Computed Style
 *
 * 计算后的样式（已解析的 CSS 属性值）
 */

#ifndef TVGBOX2_COMPUTED_STYLE_H
#define TVGBOX2_COMPUTED_STYLE_H
...
struct ComputedStyle {
   ...
   */
  Color get_variable_color(const std::string& name,
                          const Color& default_color) const;

  /**
   * 获取 CSS variable 并解析为浮点数
   */
  float get_variable_float(const std::string& name, float default_value) const;
};

} // namespace tvgbox2

#endif // TVGBOX2_COMPUTED_STYLE_H
