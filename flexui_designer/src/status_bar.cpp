...
void StatusBar::render(flex::Renderer& renderer) {
    ...
    renderer.draw_rect(x_, y_, width_, height_, 0, bg, flex::Paint::none(), 0);
    renderer.draw_rect(x_, y_, width_, 1, 0, border, flex::Paint::none(), 0);
    ...
    if (designer_) {
        ...
        if (w) {
            ...
            renderer.draw_text(buf, x_ + 10, text_y, "sans", 11, false, highlight);
        } else {
            size_t count = designer_->selection().size();
            if (count > 1) {
                char buf[64];
                snprintf(buf, sizeof(buf), "%zu widgets selected", count);
                renderer.draw_text(buf, x_ + 10, text_y, "sans", 11, false, text_col);
            } else {
                renderer.draw_text("No selection", x_ + 10, text_y, "sans", 11, false, text_col);
            }
        }

        // Center: message (temporary)
        if (message_time_ > 0 && !message_.empty()) {
            float msg_x = x_ + width_ / 2 - message_.length() * 3;
