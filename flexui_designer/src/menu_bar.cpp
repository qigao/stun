...
void MenuBar::render(flex::Renderer& renderer) {
    ...
    renderer.draw_rect(x_, y_, width_, HEIGHT, 0, bg, flex::Paint::none(), 0);

    // Bottom border
    renderer.draw_rect(x_, y_ + HEIGHT - 1, width_, 1, 0, 
    ...
    for (size_t i = 0; i < menus_.size(); ++i) {
        ...
        if ((int)i == open_menu_) {
            ...
            renderer.draw_rect(mx - 4, y_ + 2, mw + 8, HEIGHT - 4, 3, sel, flex::Paint::none(), 0);
        }

        flex::Color col = ((int)i == open_menu_) ? flex::Color{1, 1, 1, 1} : flex::Color{0.8f, 0.8f, 0.85f, 1};
        renderer.draw_text(menus_[i].label, mx, y_ + 18, "sans", 12, false, col);
        mx += mw + MENU_PADDING;
    }

    // Render open dropdown
    if (open_menu_ >= 0 && open_menu_ < (int)menus_.size()) {
        render_dropdown(renderer, menus_[open_menu_], menu_x(open_menu_));
    }
}

void MenuBar::render_dropdown(flex::Renderer& renderer, const Menu& menu, float dx) {
    float dy = y_ + HEIGHT;
    float dh = 8.0f; // padding
    for (const auto& item : menu.items) {
        dh += item.separator ? 8.0f : ITEM_HEIGHT;
    }

    // Shadow
    flex::Paint shadow = flex::Paint::solid(flex::Color{0, 0, 0, 0.3f});
    renderer.draw_rect(dx + 2, dy + 2, DROPDOWN_WIDTH, dh, 4, shadow, flex::Paint::none(), 0);
    ...
    renderer.draw_rect(dx, dy, DROPDOWN_WIDTH, dh, 4, bg, border, 1);
    ...
    for (size_t i = 0; i < menu.items.size(); ++i) {
        ...
        if (item.separator) {
            renderer.draw_rect(dx + 8, iy + 3, DROPDOWN_WIDTH - 16, 1, 0,
        ...
        if ((int)i == hovered_item_) {
            ...
            renderer.draw_rect(dx + 4, iy, DROPDOWN_WIDTH - 8, ITEM_HEIGHT - 2, 3, hover, flex::Paint::none(), 0);
        }

        // Checkmark
        if (item.checkable && item.checked) {
            renderer.draw_text("✓", dx + 10, iy + 17, "sans", 11, false, {0.4f, 0.8f, 0.5f, 1});
        }

        // Label
        float label_x = item.checkable ? dx + 28 : dx + 12;
        renderer.draw_text(item.label, label_x, iy + 17, "sans", 11, false, {0.9f, 0.9f, 0.92f, 1});

        // Shortcut
        if (!item.shortcut.empty()) {
            renderer.draw_text(item.shortcut, dx + DROPDOWN_WIDTH - 70, iy + 17, "sans", 10, false, {0.5f, 0.5f, 0.55f, 1});
        }
