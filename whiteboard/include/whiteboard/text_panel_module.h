...
public:
  ...
  // Callback types
  using ColorCallback = std::function<void(NVGcolor)>;
  using FontSizeCallback = std::function<void(float)>;
  using AlignCallback = std::function<void(int)>;
  using FontFaceCallback = std::function<void(std::string)>;

  // Setter methods
  void set_color_callback(ColorCallback cb) { m_color_callback = cb; }
  void set_font_size_callback(FontSizeCallback cb) { m_font_size_callback = cb; }
  void set_align_callback(AlignCallback cb) { m_align_callback = cb; }
  void set_font_face_callback(FontFaceCallback cb) { m_font_face_callback = cb; }
...
private:
  ...
  // Callbacks
  ColorCallback m_color_callback;
  FontSizeCallback m_font_size_callback;
  AlignCallback m_align_callback;
  FontFaceCallback m_font_face_callback;
};
