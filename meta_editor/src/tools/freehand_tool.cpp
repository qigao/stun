...
void FreehandTool::render_overlay(flex::Renderer& renderer) {
    if (!is_drawing_ || points_.size() < 2) return;
    
    flex::Paint stroke = flex::Paint::solid(flex::Color(
        stroke_color_.r, stroke_color_.g, stroke_color_.b, 0.7f));
        ...
        for (size_t i = start + 1; i < end; ++i) {
            ...
            if (len_sq < 0.0001f) {
                ...
                float t = ((points_[i].x() - points_[start].x()) * dx +
                          (points_[i].y() - points_[start].y()) * dy) / len_sq;
