...
void ShortcutPanel::render(flex::Renderer& renderer) {
    ...
    // Title
    renderer.draw_text("Keyboard Shortcuts", x_ + width_ / 2 - 80, y_ + 30, 
                       "Arial", 16.0f, true, flex::Color{1.0f, 1.0f, 1.0f, 1.0f});

    // Close hint
    renderer.draw_text("Press ? or Esc to close", x_ + width_ / 2 - 70, y_ + 50,
                       "Arial", 10.0f, false, flex::Color{0.6f, 0.6f, 0.6f, 1.0f});
    ...
    for (const auto& s : shortcuts_) {
        ...
        if (s.category != current_category) {
            ...
            renderer.draw_text(current_category.c_str(), x_ + 20, row_y,
                              "Arial", 12.0f, true, flex::Color{0.5f, 0.7f, 0.9f, 1.0f});
            row_y += 22;
        }
        ...
        // Description
        renderer.draw_text(s.description.c_str(), x_ + 40 + key_width + 10, row_y,
                          "Arial", 11.0f, false, flex::Color{0.8f, 0.8f, 0.8f, 1.0f});
