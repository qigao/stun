...
**当前代码** (`nanovg_css_painter.cpp:1800`):
```cpp
// ❌ 每帧都分配字符串
...
**或者更好的方案**（修改 NVGCSSElement）:
```cpp
struct NVGCSSElement {
    // ✅ 使用智能指针
    nvgcss::PoolPtr<TransitionState> transition_state;
    nvgcss::PoolPtr<AnimationState> animation_state;
};
```
...
```cpp
void print_memory_stats(NVGCSSRenderer* renderer) {
...
```cpp
// 每帧检查
void check_memory_leak(NVGCSSRenderer* renderer) {
