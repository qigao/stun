...
int main() {
    ...
    auto* dots_pattern = nvgcssCreatePattern(renderer, "dots", 0, 0, 20, 20);
    auto* dot = nvgcssCreateElement(renderer, "dot1", "circle");
    ...
    nvgcssAppendChild(renderer, dots_pattern, dot);

    // Create a stripes pattern
    auto* stripes_pattern = nvgcssCreatePattern(renderer, "stripes", 0, 0, 10, 10);
    auto* stripe = nvgcssCreateElement(renderer, "stripe1", "rect");
    ...
    nvgcssAppendChild(renderer, stripes_pattern, stripe);

    // Rectangle with dots pattern
    auto* pattern_rect1 = nvgcssCreateElement(renderer, "pr1", "rect");
    ...
    auto* pattern_circle = nvgcssCreateElement(renderer, "pc1", "circle");
    ...
    auto* curve_path = nvgcssCreateElement(renderer, "curve1", "path");
    ...
    auto* text_path1 = nvgcssCreateElement(renderer, "tp1", "textPath");
    ...
    auto* wave_path = nvgcssCreateElement(renderer, "wave1", "path");
    ...
    auto* text_path2 = nvgcssCreateElement(renderer, "tp2", "textPath");
    ...
    // Create arrow marker
    auto* arrow_marker = nvgcssCreateMarker(renderer, "arrow", 10, 10, 5, 5, "auto");
    auto* arrow_path = nvgcssCreateElement(renderer, "arrow_shape", "path");
    arrow_path->inline_style["d"] = "M 0 0 L 10 5 L 0 10 Z";
    arrow_path->inline_style["fill"] = "#27ae60";
    nvgcssAppendChild(renderer, arrow_marker, arrow_path);

    // Create dot marker
    auto* dot_marker = nvgcssCreateMarker(renderer, "dot", 8, 8, 4, 4, "0");
    auto* marker_circle = nvgcssCreateElement(renderer, "marker_dot", "circle");
    ...
    nvgcssAppendChild(renderer, dot_marker, marker_circle);

    // Path with markers
    auto* marker_path = nvgcssCreateElement(renderer, "mp1", "path");
    ...
    // Create clip path
    auto* clip = nvgcssCreateClipPath(renderer, "clip1");
    auto* clip_rect = nvgcssCreateElement(renderer, "clip_shape", "rect");
    ...
    nvgcssAppendChild(renderer, clip, clip_rect);

    // Element with clipping
    auto* clipped_circle = nvgcssCreateElement(renderer, "cc1", "circle");
    ...
    auto* label4 = nvgcssCreateElement(renderer, "l4", "div");
    ...
    while (running) {
        ...
        nvgcssUpdate(renderer, 0.016f);  // 60fps delta time
