...
".section-title { font-size: 18px; font-weight: bold; color: #34495e; position: absolute; }\n";
        ...
        nvgcssSetText(title, "Advanced CSS Features Demo");

        // Section 1: Complex Gradients
        auto sec1 = nvgcssCreateElement(renderer, "sec1", "div");
        nvgcssAddClass(sec1, "section-title");
        sec1->inline_style["left"] = "50px";
        sec1->inline_style["top"] = "90px";
        nvgcssSetText(sec1, "Complex Gradients (hover to scale)");

        auto grad1 = nvgcssCreateElement(renderer, "grad1", "div");
        nvgcssAddClass(grad1, "gradient-box");
        grad1->inline_style["left"] = "50px";
        grad1->inline_style["top"] = "130px";

        auto grad2 = nvgcssCreateElement(renderer, "grad2", "div");
        nvgcssAddClass(grad2, "gradient-box");
        ...
        auto grad3 = nvgcssCreateElement(renderer, "grad3", "div");
        nvgcssAddClass(grad3, "gradient-box");
        ...
        auto grad4 = nvgcssCreateElement(renderer, "grad4", "div");
        nvgcssAddClass(grad4, "gradient-box");
        ...
        auto sec2 = nvgcssCreateElement(renderer, "sec2", "div");
        nvgcssAddClass(sec2, "section-title");
        sec2->inline_style["left"] = "50px";
        sec2->inline_style["top"] = "280px";
        nvgcssSetText(sec2, "Layered Shadows (hover for depth)");

        auto shadow1 = nvgcssCreateElement(renderer, "shadow1", "div");
        nvgcssAddClass(shadow1, "shadow-card");
        shadow1->inline_style["left"] = "50px";
        shadow1->inline_style["top"] = "320px";

        auto shadow2 = nvgcssCreateElement(renderer, "shadow2", "div");
        nvgcssAddClass(shadow2, "shadow-card");
        shadow2->inline_style["left"] = "280px";
        shadow2->inline_style["top"] = "320px";

        auto shadow3 = nvgcssCreateElement(renderer, "shadow3", "div");
        nvgcssAddClass(shadow3, "shadow-card");
        shadow3->inline_style["left"] = "510px";
        shadow3->inline_style["top"] = "320px";

        // Section 3: SVG Shapes
        auto sec3 = nvgcssCreateElement(renderer, "sec3", "div");
        nvgcssAddClass(sec3, "section-title");
        sec3->inline_style["left"] = "750px";
        sec3->inline_style["top"] = "90px";
        nvgcssSetText(sec3, "SVG Shapes");
        ...
        // Polygon (Star)
        auto polygon = nvgcssCreateElement(renderer, "polygon1", "polygon");
        ...
        // Polyline (Zigzag)
        auto polyline = nvgcssCreateElement(renderer, "polyline1", "polyline");
        ...
        logi("After layout:");
        logi("grad1: x={}, y={}, w={}, h={}", grad1->computed.x, grad1->computed.y, grad1->computed.width, grad1->computed.height);
        logi("shadow1: x={}, y={}, w={}, h={}", shadow1->computed.x, shadow1->computed.y, shadow1->computed.width, shadow1->computed.height);
    }
    ...
    void update_hover() {
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            auto item = nvgcssGetElement(renderer, id);
            if (item) {
                float x = item->computed.x;
                float y = item->computed.y;
                float w = item->computed.width;
                float h = item->computed.height;
                bool hover = (mouse_x >= x && mouse_x <= x + w &&
                             mouse_y >= y && mouse_y <= y + h);
                nvgcssSetPseudoState(item, "hover", hover ? 1 : 0);
            }
        }
    ...
    void render() {
        logi("[RENDER LOOP] Starting render, mouse=({}, {})", mouse_x, mouse_y);
