...
private:
    ...
    void init_nanovg() {
        ...
        nvgcssSetViewport(renderer, 1280, 800);
    }
    ...
    void check_hover(const char* id, int mx, int my, float x, float y, float w, float h) {
        NVGCSSElement* elem = nvgcssGetElement(renderer, id);
        if (!elem) return;

        bool is_inside = (mx >= x && mx <= x + w && my >= y && my <= y + h);
        nvgcssSetPseudoState(elem, "hover", is_inside ? 1 : 0);
    }
