...
static void mapAttributes(NVGCSSElement* element, pugi::xml_node xml_node) {
    for (pugi::xml_attribute attr : xml_node.attributes()) {
        ...
        if (mapping) {
            ...
            // CSS classes
            nvgcssAddClass(element, attr_value);
...
static NVGCSSElement* parseNode(
    NVGCSSRenderer* renderer,
    pugi::xml_node xml_node,
    NVGCSSElement* parent,
    const nvgcss::SVGXMLParser::Options& options
) {
    // Get element type
    ...
    // Create element
    NVGCSSElement* element = nvgcssCreateElement(renderer, element_id.c_str(), element_type);
    if (!element) {
    ...
    if (std::strcmp(element_type, "svg") == 0 ||
        ...
        nvgcssAddClass(element, "svg-element");
    }
    ...
    if (std::strcmp(tag_name, "text") == 0) {
        ...
        if (text && text[0] != '\0') {
            nvgcssSetText(element, text);
        }
    }

    // Attach to parent
    if (parent) {
        nvgcssAppendChild(renderer, parent, element);
    }
    ...
    if (std::strcmp(tag_name, "linearGradient") == 0 || std::strcmp(tag_name, "radialGradient") == 0) {
        ...
        for (pugi::xml_node stop_node : xml_node.children("stop")) {
            ...
            if (color_attr) {
                stop.color = nvgcss_utils::parse_color(color_attr.value());
            }
...
NVGCSSElement* nvgcssLoadSVG(
    ...
    // Parse tree
    nvgcss::SVGXMLParser::Options options;
    element_counter = 0;  // Reset counter
    NVGCSSElement* root = parseNode(renderer, svg_root, parent, options);
    
    // Build gradient cache for all elements (performance optimization)
    if (root) {
        nvgcss::SVGXMLParser::resolve_gradient_references(renderer, root);
    }
    
    return root;
}

NVGCSSElement* nvgcssLoadSVGFile(
    NVGCSSRenderer* renderer,
    const char* filepath,
    NVGCSSElement* parent
) {
    ...
    return nvgcssLoadSVG(renderer, content.c_str(), parent);
}
...
NVGCSSElement* SVGXMLParser::parse(
    NVGCSSRenderer* renderer,
...
NVGCSSElement* SVGXMLParser::parseFile(
    NVGCSSRenderer* renderer,
 ...
 */
void SVGXMLParser::resolve_gradient_references(NVGCSSRenderer* renderer, NVGCSSElement* element) {
