...
static std::string get_string_property(NVGCSSRenderer* renderer, NVGCSSElement* element, const std::string& prop) {
...
LayoutNode* QuadtreeLayoutEngine::build_tree(NVGCSSElement* root, NVGCSSRenderer* renderer) {
    ...
    // NEW: Populate explicit_style from typed properties for backward compatibility
    // This ensures tests that check explicit_style values pass
    if (!root->style.width.is_auto()) {
        root->explicit_style.width = root->style.width.resolve(viewport_width_, 16, viewport_width_);
    }
    if (!root->style.height.is_auto()) {
        root->explicit_style.height = root->style.height.resolve(viewport_height_, 16, viewport_height_);
    }
    

    
    // Populate box model properties in explicit_style
    for (int i = 0; i < 4; i++) {
        root->explicit_style.padding[i] = node->constraints.padding[i];
        root->explicit_style.margin[i] = node->constraints.margin[i];
        root->explicit_style.border_width[i] = node->constraints.border[i];
        // Border radius - resolve from typed property
        root->explicit_style.border_radius[i] = root->style.border.radius[i];
    }
    ...
    // Populate x/y positions (from left/top properties or custom x/y)
    if (!root->style.left.is_auto()) {
        root->explicit_style.x = root->style.left.resolve(viewport_width_, 16, viewport_width_);
    } else {
        auto it = computed_style.find("x");
        if (it != computed_style.end()) {
            root->explicit_style.x = nvgcssParseLength(it->second.c_str(), viewport_width_);
        }
    }

    if (!root->style.top.is_auto()) {
        root->explicit_style.y = root->style.top.resolve(viewport_height_, 16, viewport_height_);
    } else {
        auto it = computed_style.find("y");
        if (it != computed_style.end()) {
            root->explicit_style.y = nvgcssParseLength(it->second.c_str(), viewport_height_);
        }
    }
    ...
    int child_count = 0;
    NVGCSSElement** children_ptr = nvgcssGetChildren(renderer, root, &child_count);
...
void QuadtreeLayoutEngine::compute_layout(LayoutNode* root, NVGCSSRenderer* renderer) {
    if (!root) return;
...
void QuadtreeLayoutEngine::calculate_dimensions(LayoutNode* node, NVGCSSRenderer* renderer) {
...
void QuadtreeLayoutEngine::calculate_block_dimensions(LayoutNode* node, NVGCSSRenderer* renderer) {
...
void QuadtreeLayoutEngine::calculate_flex_dimensions(LayoutNode* node, NVGCSSRenderer* renderer) {
...
void QuadtreeLayoutEngine::calculate_grid_dimensions(LayoutNode* node, NVGCSSRenderer* renderer) {
    // Sort children by order
    if (!node->children.empty()) {
        std::stable_sort(node->children.begin(), node->children.end(), 
            [](const LayoutNode* a, const LayoutNode* b) {
...
void QuadtreeLayoutEngine::calculate_positions(LayoutNode* node, float parent_x, float parent_y) {
    ...
    // Override with explicit position if set (e.g. from inline style "x" or "left")
    // This treats explicit x/y as relative to parent (or absolute for root)
    if (node->element->explicit_style.x != -1.0f) {
        float parent_abs_x = node->parent ? node->parent->x : 0.0f;
        node->x = parent_abs_x + node->element->explicit_style.x;
    }
    
    if (node->element->explicit_style.y != -1.0f) {
        float parent_abs_y = node->parent ? node->parent->y : 0.0f;
        node->y = parent_abs_y + node->element->explicit_style.y;
    }
...
void QuadtreeLayoutEngine::write_to_elements(LayoutNode* root) {
    ...
    // Write computed layout to element
    root->element->computed.x = root->x;
    root->element->computed.y = root->y;
    root->element->computed.width = root->width;
    root->element->computed.height = root->height;
    root->element->computed.content_width = root->content_width;
    root->element->computed.content_height = root->content_height;
    ...
    for (const auto& cls : root->element->classes) {
        if (cls == "stat-card" || cls == "stat-label" || cls == "stat-value" || cls == "stats-row") {
            logi("[POSITION] class='{}' size=({:.0f}x{:.0f}) height_auto={}",
                 cls, root->width, root->height, root->constraints.height.is_auto());
            break;
        }
    }

    // NEW: Copy box model to computed (tests expect this)
    for (int i = 0; i < 4; i++) {
        root->element->computed.padding[i] = root->constraints.padding[i];
        root->element->computed.margin[i] = root->constraints.margin[i];
        root->element->computed.border[i] = root->constraints.border[i];
        root->element->computed.border_radius[i] = root->element->explicit_style.border_radius[i];
    }
    
    // Set source based on how the element was positioned
    // FIXED: Use CSS_EXPLICIT for elements with explicit CSS dimensions or simple block layout
    bool has_explicit_dims = !root->element->style.width.is_auto() || 
                             !root->element->style.height.is_auto() ||
                             !root->element->style.left.is_auto() ||
                             !root->element->style.top.is_auto();
    
    if (root->parent) {
        if (root->parent->type == LayoutNode::GRID) {
            root->element->computed.source = NVGCSSComputedLayout::GRID;
        } else if (root->parent->type == LayoutNode::FLEX) {
            root->element->computed.source = NVGCSSComputedLayout::FLEXBOX;
        } else if (has_explicit_dims) {
            root->element->computed.source = NVGCSSComputedLayout::CSS_EXPLICIT;
        } else {
            root->element->computed.source = NVGCSSComputedLayout::FLOW;
        }
    } else {
        // Root element itself
        if (root->type == LayoutNode::GRID) {
            root->element->computed.source = NVGCSSComputedLayout::GRID;
        } else if (root->type == LayoutNode::FLEX) {
            root->element->computed.source = NVGCSSComputedLayout::FLEXBOX;
        } else {
            // Default to CSS_EXPLICIT for simple block elements (not FLOW)
            root->element->computed.source = NVGCSSComputedLayout::CSS_EXPLICIT;
        }
    }
