...
struct NVGCSSFilterContext {
    GLuint fbo;
    GLuint texture;
    GLuint vao, vbo;
    int width, height;
    std::map<NVGCSSFilterType, GLuint> shaders;
    
    NVGCSSFilterContext() : fbo(0), texture(0), vao(0), vbo(0), width(0), height(0) {}
};
...
NVGCSSFilterContext* nvgcssCreateFilterContext(void) {
    NVGCSSFilterContext* ctx = new NVGCSSFilterContext();
    ...
    ctx->shaders[NVGCSS_FILTER_BLUR] = compile_shader(vertex_shader_src, (base + blur_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_BRIGHTNESS] = compile_shader(vertex_shader_src, (base + brightness_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_CONTRAST] = compile_shader(vertex_shader_src, (base + contrast_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_GRAYSCALE] = compile_shader(vertex_shader_src, (base + grayscale_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_HUE_ROTATE] = compile_shader(vertex_shader_src, (base + hue_rotate_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_INVERT] = compile_shader(vertex_shader_src, (base + invert_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_SATURATE] = compile_shader(vertex_shader_src, (base + saturate_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_SEPIA] = compile_shader(vertex_shader_src, (base + sepia_shader).c_str());
    ctx->shaders[NVGCSS_FILTER_OPACITY] = compile_shader(vertex_shader_src, (base + opacity_shader).c_str());
...
static void resize_fbo(NVGCSSFilterContext* ctx, int width, int height) {
    ...
    void* user_data)
{
    ...
    for (int i = 0; i < filter_count; i++) {
        const NVGCSSFilter& filter = filters[i];
