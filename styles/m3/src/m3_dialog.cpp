...
void M3Dialog::layout_actions() {
    ...
    // Horizontal layout for �? actions, vertical for >2
...
void M3Dialog::draw(NVGcontext *ctx) {
    ...
    nvgSave(ctx);
    nvgResetScissor(ctx);  // Ensure dialog isn't clipped
    ...
    // Draw dialog background with proper anti-aliasing
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, x, y, w, h, corner_radius);
    ...
    // Add a subtle stroke to clean up the edges
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, x + 0.5f, y + 0.5f, w - 1.0f, h - 1.0f, corner_radius - 0.5f);
    nvgStrokeWidth(ctx, 1.0f);
    nvgStrokeColor(ctx, theme->surface());
    nvgStroke(ctx);
