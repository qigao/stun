...
void Path::add_point(const PathPoint &point) { points_.push_back(point); }

bool Path::is_valid() const {
  if (mode_ != InterpolationMode::Bezier) {
    return points_.size() >= 2;
  }

  if (points_.size() < 4 || (points_.size() - 1) % 3 != 0) {
    return false;
  }
  for (size_t endpoint = 3; endpoint < points_.size(); endpoint += 3) {
    if (points_[endpoint].time <= points_[endpoint - 3].time) {
      return false;
    }
  }
  return true;
...
PathPoint Path::interpolate(float t) const {
  ...
  case InterpolationMode::Bezier:
    return interpolate_bezier(t);
  default:
    return interpolate_linear(t);
  }
}

PathPoint Path::interpolate_bezier(float t) const {
  if (t <= points_.front().time) {
    return PathPoint(points_.front().x, points_.front().y, t);
  }

  size_t endpoint = 3;
  while (endpoint < points_.size() - 1 && t > points_[endpoint].time) {
    endpoint += 3;
  }

  if (t >= points_[endpoint].time && endpoint == points_.size() - 1) {
    return PathPoint(points_[endpoint].x, points_[endpoint].y, t);
  }

  const PathPoint &p0 = points_[endpoint - 3];
  const PathPoint &p1 = points_[endpoint - 2];
  const PathPoint &p2 = points_[endpoint - 1];
  const PathPoint &p3 = points_[endpoint];
  const float duration = p3.time - p0.time;
  const float parameter = (t - p0.time) / duration;
  const CubicBezier2D curve({p0.x, p0.y}, {p1.x, p1.y}, {p2.x, p2.y}, {p3.x, p3.y});
  const Vec2 position = curve.position(parameter);
  return PathPoint(position.x, position.y, t);
}
...
PathPoint Path::interpolate_catmull_rom(float t) const {
  ...
  float x = 0.5f * ((2.0f * p1.x) + (-p0.x + p2.x) * local_t +
                    (2.0f * p0.x - 5.0f * p1.x + 4.0f * p2.x - p3.x) * t2 +
                    (-p0.x + 3.0f * p1.x - 3.0f * p2.x + p3.x) * t3);

  // Calculate y coordinate
  float y = 0.5f * ((2.0f * p1.y) + (-p0.y + p2.y) * local_t +
...
PathAnimation::PathAnimation(Path *path, float duration)
    : path_(path), duration_(duration), elapsed_(0), playing_(false), loop_(false) {}
