...
void Node::update_world_transform() {
    if (is_dirty(DirtyFlags::Transform)) {
        ...
        if (position_mode() == PositionMode::Fixed || !parent_) {
...
void Node::mark_topology_changed() {
    for (Node* node = this; node; node = node->parent_) {
        if (node->topology_revision_ == (std::numeric_limits<uint64_t>::max)()) {
            node->topology_revision_ = 1;
        } else {
            ++node->topology_revision_;
        }
    }
}
...
bool Node::intersects_viewport(const Bounds& viewport) const {
    ...
    bool intersects = !(b.x + b.width < viewport.x ||
    ...
    auto* group = static_cast<const Group*>(this);
    for (auto* child : group->children()) {
        if (!child || !child->visible()) {
            continue;
        }

        if (group->clip() && child->position_mode() != PositionMode::Fixed) {
            continue;
        }

        if (child->intersects_viewport(viewport)) {
            return true;
        }
    }
...
void Node::fire_focus_event(FocusEvent& event) {
    if (events_ && events_->on_focus_event) events_->on_focus_event(event);
}

void Node::fire_text_input(TextInputEvent& event) {
    if (events_ && events_->on_text_input) events_->on_text_input(event);
}

void Node::fire_composition(CompositionEvent& event) {
    if (events_ && events_->on_composition) events_->on_composition(event);
}
...
bool Node::set_animated_property(PropertyID pid, const AnimValue& value) {
    if (pid == PropertyID::Position) {
        const auto* position = std::get_if<Vec2>(&value);
        if (!position) return false;
        if (x_ != position->x || y_ != position->y) {
