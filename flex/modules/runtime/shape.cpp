...
void Shape::render(Renderer &r) {
  ...
  // -------------------------------------------
  if (!is_rough && !has_shadow() && !has_blur() && r.supports_retained_mode()) {
    Transform t = world_transform();
    bool transform_changed = !has_cached_paint_transform() || cached_paint_transform() != t;
    bool needs_rebuild = is_dirty(DirtyFlags::Content) || is_dirty(DirtyFlags::Visual);

    // Already cached and no changes? Skip entirely!
    if (cached_paint() && !needs_rebuild) {
      if (transform_changed) {
        r.update_transform(cached_paint(), t);
        set_cached_paint_transform(t);
    ...
    if (new_paint) {
      ...
      set_cached_paint_transform(t);
      clear_dirty(DirtyFlags::Content | DirtyFlags::Visual | DirtyFlags::Transform);
      return;
    }
  ...
  if (!is_rough) {
      ...
      if (has_shadow()) r.set_shadow(shadow());
      if (has_blur()) r.set_blur(blur());
