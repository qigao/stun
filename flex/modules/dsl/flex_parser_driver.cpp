 ...
 *
 * Design: Lemon validates syntax while the event-driven AstBuilder below is
 * the single owner of semantic state and AST construction.
 */
...
static thread_local int g_last_error_line = 0;
static thread_local int g_last_error_column = 0;
...
public:
  ...
  void on_token(const Token &tok, const Token *prev_tok) {
    ...
    case TOK_COMMA:
      if ((expecting_value_ || expecting_keyframe_value_ || expecting_const_value_) &&
          value_parenthesis_depth_ > 0) {
    ...
    case TOK_RPAREN:
      ...
      // Handle asset option value
        ...
        if (tok.type == TOK_NUMBER) {
          float parsed = 0.0f;
          if (!parse_number(tok, false, parsed)) {
            break;
          }
          value = parsed;
      ...
      // Handle repeat count 
        ...
        if (parsed < 1.0f || parsed > static_cast<float>(kMaxRepeatCount) ||
            std::floor(parsed) != parsed) {
          set_error(tok, "repeat count must be an integer between 1 and " +
                             std::to_string(kMaxRepeatCount));
          break;
        }
        pending_repeat_count_ = static_cast<int>(parsed);
      ...
      else if (expecting_trigger_event_ && tok.type == TOK_STRING) {
        if (current_anim_) {
          current_anim_->triggers.push_back({pending_trigger_time_, tok.value});
        }
        expecting_trigger_event_ = false;
      }
      ...
      if ((expecting_value_ || expecting_keyframe_value_ || expecting_const_value_) &&
          tok.type == TOK_RPAREN && value_parenthesis_depth_ > 0) {
        --value_parenthesis_depth_;
      }
      break;
    ...
    case TOK_TRACK:
      // Animation properties do not require commas; a track starts the next
      // item and therefore closes the preceding duration/loop value.
      if (expecting_value_ && !pending_value_tokens_.empty()) {
        flush_pending_value();
      }
    ...
    case TOK_KEYFRAME:
      // Likewise, a keyframe closes an optional track expression property.
      if (expecting_value_ && !pending_value_tokens_.empty()) {
        flush_pending_value();
      }
    ...
    case TOK_TRIGGER:
      ...
      expecting_trigger_time_ = true;
      break;

    case TOK_ARROW:
      if (expecting_keyframe_time_ && prev_tok && prev_tok->type == TOK_NUMBER) {
        if (!parse_number(*prev_tok, true, pending_keyframe_time_)) {
          break;
        }
        expecting_keyframe_time_ = false;
        expecting_keyframe_value_ = true;
      } else if (expecting_trigger_time_ && prev_tok && prev_tok->type == TOK_NUMBER) {
        if (!parse_number(*prev_tok, true, pending_trigger_time_)) {
          break;
        }
        expecting_trigger_time_ = false;
        expecting_trigger_event_ = true;
      }
      break;
...
private:
  static constexpr int kMaxRepeatCount = 10000;

  void set_error(const Token &token, std::string message) {
    if (has_error()) {
      return;
    }
  ...
  bool parse_number(const Token &token, bool allow_time_unit, float &value) {
    std::string number = token.value;
    float scale = 1.0f;
    bool has_unit = false;
    if (number.size() >= 2 &&
        (number.compare(number.size() - 2, 2, "ms") == 0 ||
         number.compare(number.size() - 2, 2, "MS") == 0 ||
         number.compare(number.size() - 2, 2, "mS") == 0 ||
         number.compare(number.size() - 2, 2, "Ms") == 0)) {
      ...
      has_unit = true;
               ...
               (number.back() == 's' || number.back() == 'S')) {
      number.pop_back();
      has_unit = true;
    }

    if (has_unit && !allow_time_unit) {
      set_error(token, "time unit is only valid for animation duration and keyframe time");
      return false;
    }
    ...
    if (end != number.c_str() + number.size() || errno == ERANGE ||
        !std::isfinite(parsed)) {
      set_error(token, "invalid numeric literal '" + token.value + "'");
      return false;
    }
    value = parsed * scale;
    if (!std::isfinite(value)) {
      set_error(token, "numeric literal is outside the supported range: '" +
                           token.value + "'");
      return false;
    }
    return true;
  ...
  void handle_identifier(const Token &tok, const Token *prev_tok) {
    ...
    } else if (expecting_keyframe_value_ || expecting_value_) {
  ...
  bool try_evaluate_expression(const std::vector<Token>& tokens, float& result) {
    ...
    for (const auto& tok : tokens) {
      if (tok.type == TOK_IDENTIFIER) {
        ...
        expr += std::to_string(parsed);
      } else if (tok.type == TOK_PLUS || tok.type == TOK_MINUS || tok.type == TOK_STAR ||
    ...
    Expr evaluator;
    const auto evaluated = evaluator.try_eval(expr);
    if (!evaluated || !std::isfinite(*evaluated)) {
      return false;
    }
    result = static_cast<float>(*evaluated);
    return true;
  }

  bool try_parse_vec2_literal(const std::vector<Token>& tokens, AstVec2& result) {
    if (tokens.empty() || tokens.front().type != TOK_IDENTIFIER ||
        tokens.front().value != "vec2") {
      return false;
    }
    if (tokens.size() < 6 || tokens[1].type != TOK_LPAREN ||
        tokens.back().type != TOK_RPAREN) {
      set_error(tokens.front(), "vec2 requires exactly two numeric arguments");
      return false;
    }
    ...
    for (size_t index = 2; index + 1 < tokens.size(); ++index) {
      if (tokens[index].type == TOK_LPAREN) {
        ++depth;
      } else if (tokens[index].type == TOK_RPAREN) {
        --depth;
      } else if (tokens[index].type == TOK_COMMA && depth == 1) {
        if (separator != tokens.size()) {
          set_error(tokens[index], "vec2 requires exactly two numeric arguments");
          return false;
        }
        separator = index;
      }
    ...
    if (separator == tokens.size() || separator == 2 ||
        separator + 1 >= tokens.size() - 1) {
      set_error(tokens.front(), "vec2 requires exactly two numeric arguments");
      return false;
    }

    const std::vector<Token> x_tokens(tokens.begin() + 2, tokens.begin() + separator);
    const std::vector<Token> y_tokens(tokens.begin() + separator + 1, tokens.end() - 1);
    float x = 0.0f;
    float y = 0.0f;
    if (!try_evaluate_expression(x_tokens, x) || !try_evaluate_expression(y_tokens, y) ||
        !std::isfinite(x) || !std::isfinite(y)) {
      set_error(tokens.front(), "vec2 arguments must be finite numeric expressions");
      return false;
    }
    result = {x, y};
    return true;
  ...
  bool try_parse_bezier_keyframe_literal(const std::vector<Token> &tokens,
                                          AstVec2 &position,
                                          AstVec2 &in_tangent,
                                          AstVec2 &out_tangent) {
    if (tokens.empty() || tokens.front().type != TOK_IDENTIFIER ||
        tokens.front().value != "bezier") {
      return false;
    }
    if (tokens.size() < 10 || tokens[1].type != TOK_LPAREN ||
        tokens.back().type != TOK_RPAREN) {
      set_error(tokens.front(),
                "bezier requires position, inTangent, and outTangent vec2 arguments");
      return false;
    }

    size_t first_separator = tokens.size();
    size_t second_separator = tokens.size();
    int depth = 1;
    for (size_t index = 2; index + 1 < tokens.size(); ++index) {
      if (tokens[index].type == TOK_LPAREN) {
        ++depth;
        ...
        if (first_separator == tokens.size()) {
          first_separator = index;
        } else if (second_separator == tokens.size()) {
          second_separator = index;
        } else {
          set_error(tokens[index], "bezier requires exactly three vec2 arguments");
          return false;
        }
      }
    }

    if (first_separator == tokens.size() || second_separator == tokens.size() ||
        first_separator == 2 || first_separator + 1 == second_separator ||
        second_separator + 1 >= tokens.size() - 1) {
      set_error(tokens.front(), "bezier requires exactly three vec2 arguments");
      return false;
    }

    const std::vector<Token> position_tokens(tokens.begin() + 2,
                                             tokens.begin() + first_separator);
    const std::vector<Token> in_tokens(tokens.begin() + first_separator + 1,
                                       tokens.begin() + second_separator);
    const std::vector<Token> out_tokens(tokens.begin() + second_separator + 1,
                                        tokens.end() - 1);
    return try_parse_vec2_literal(position_tokens, position) &&
           try_parse_vec2_literal(in_tokens, in_tangent) &&
           try_parse_vec2_literal(out_tokens, out_tangent);
  }
  ...
  void flush_pending_value() {
     ...
     if (expecting_keyframe_value_ && pending_value_tokens_.front().type == TOK_IDENTIFIER &&
         pending_value_tokens_.front().value == "bezier") {
       AstVec2 position;
       AstVec2 in_tangent;
       AstVec2 out_tangent;
       if (try_parse_bezier_keyframe_literal(pending_value_tokens_, position,
                                             in_tangent, out_tangent)) {
         current_track_.keyframes.emplace_back(
             pending_keyframe_time_, position, in_tangent, out_tangent);
       }
       pending_value_tokens_.clear();
     ...
     if (expecting_keyframe_value_ && pending_value_tokens_.front().type == TOK_IDENTIFIER &&
         pending_value_tokens_.front().value == "vec2") {
       AstVec2 vector_value;
       if (try_parse_vec2_literal(pending_value_tokens_, vector_value)) {
         current_track_.keyframes.emplace_back(pending_keyframe_time_, vector_value);
       }
       pending_value_tokens_.clear();
       expecting_keyframe_value_ = false;
       value_parenthesis_depth_ = 0;
     ...
     if (pending_value_tokens_.size() == 1 && pending_value_tokens_[0].type != TOK_BINDING) {
         ...
         switch (tok.type) {
            case TOK_NUMBER: {
              const bool is_duration = current_anim_ && !in_track_ &&
                                       pending_prop_key_ == "duration";
              float parsed = 0.0f;
              if (!parse_number(tok, is_duration, parsed)) {
                pending_value_tokens_.clear();
                return;
              }
              value = parsed;
     ...
     if (expecting_keyframe_value_) {
         std::visit(
             [this](const auto &keyframe_value) {
               current_track_.keyframes.emplace_back(
                   pending_keyframe_time_, AstKeyframeValue{keyframe_value});
             },
             value);
         expecting_keyframe_value_ = false;
  ...
  void expand_for_node(const std::shared_ptr<AstNode> &for_node,
    ...
    if (!data) {
      set_error("for loop references unknown data block '" + *source + "'");
      return;
    }
  ...
  void set_property(const AstValue &value) {
    ...
    } else if (current_anim_ && in_track_) {
      if (pending_prop_key_ == "interpolation") {
        const auto *interpolation = std::get_if<std::string>(&value);
        if (!interpolation || (*interpolation != "linear" && *interpolation != "catmullRom" &&
                               *interpolation != "cubicBezier")) {
          set_error(
              "animation track interpolation must be 'linear', 'catmullRom', or 'cubicBezier'");
        } else {
          if (*interpolation == "catmullRom") {
            current_track_.spatial_interpolation = AstSpatialInterpolation::CatmullRom;
          } else if (*interpolation == "cubicBezier") {
            current_track_.spatial_interpolation = AstSpatialInterpolation::CubicBezier;
          } else {
            current_track_.spatial_interpolation = AstSpatialInterpolation::Linear;
          }
        }
      } else if (pending_prop_key_ != "expression") {
        set_error("unknown animation track property '" + pending_prop_key_ + "'");
      } else if (auto *expression = std::get_if<std::string>(&value)) {
        current_track_.numeric_expression = normalize_binding_expression(*expression);
      } else {
        set_error("animation track expression must use ${...} or $(...)");
      }
  ...
  float pending_trigger_time_ = 0;
  int value_parenthesis_depth_ = 0;
  ...
  bool expecting_trigger_time_ = false;
  bool expecting_trigger_event_ = false;
...
std::shared_ptr<AstProgram> parse(const char *source) {
  ...
  if (!parser) {
    lexer_destroy(lexer);
    g_last_error = "Could not allocate DSL parser";
    return nullptr;
  }
  ...
  do {
    ...
    if (builder.has_error()) {
      g_last_error = builder.error_message();
      g_last_error_line = builder.error_line();
      g_last_error_column = builder.error_column();
      ParseFree(parser, free);
      lexer_destroy(lexer);
      return nullptr;
    }
