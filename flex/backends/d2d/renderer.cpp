...
class D2DRenderer final : public Renderer {
 public:
  ...
  void set_shadow(const Shadow& shadow) override {
    if (!std::isfinite(shadow.offset_x) || !std::isfinite(shadow.offset_y) ||
      ...
      throw std::invalid_argument(
          "Direct2D renderer shadow values must be finite with non-negative blur");
    }
    if (!shadow.is_none() && shadow.color.a > 0.0f) {
      throw std::logic_error("Direct2D renderer does not support shadows");
    }
  }
  ...
  void set_blur(const BlurFilter& blur) override {
    if (!std::isfinite(blur.radius) || blur.radius < 0.0f) {
      throw std::invalid_argument(
          "Direct2D renderer blur radius must be finite and non-negative");
    }
    if (!blur.is_none()) {
      throw std::logic_error("Direct2D renderer does not support blur filters");
    }
  }
 ...
 private:
  ...
  CachedBitmap ensure_svg_bitmap(const std::string& key, const ::gcanvas::SvgDocument& document,
    ...
    std::vector<std::uint8_t> premultiplied(bitmap.rgba.size());
    const std::uint8_t* src = bitmap.rgba.data();
