...
struct ResizeState {
  ...
  void begin(ResizeHandle h, flex::Node *n, float wx, float wy, float x, float y, float w,
             float height) {
    active = true;
    handle = h;
    node = n;
...
struct DrawState {
  ...
  std::string to_path(bool smooth = false) const { return to_path_relative(0, 0, smooth); }
...
struct Toolbar {
  ...
  void init() {
    tools = {
        {ToolType::Select, "V", "Select (V)"},       {ToolType::Pan, "H", "Pan (H)"},
        {ToolType::Rectangle, "R", "Rectangle (R)"}, {ToolType::Circle, "O", "Circle (O)"},
        {ToolType::Line, "L", "Line (L)"},           {ToolType::Pen, "B", "Pen (B)"},
    };
    commands = {
  ...
  float tools_end_x() const { return PADDING + tools.size() * (BUTTON_SIZE + PADDING); }

  float commands_start_x() const { return tools_end_x() + SEPARATOR; }
...
struct OverviewWindow {
  ...
  float y(float screen_height) const { return screen_height - height - margin; }

  // Convert screen coords in overview to world coords
  void overview_to_world(float ox, float oy, float overview_x, float overview_y, float &wx,
                         float &wy) const {
  ...
  bool hit_test(float mx, float my, float overview_x, float overview_y) const {
    return mx >= overview_x && mx < overview_x + width && my >= overview_y &&
           my < overview_y + height;
  }
};
...
public:
  ...
  bool init(const char *file = nullptr) {
    ...
    window_ =
        SDL_CreateWindow("Flex Meta-Editor Demo", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    tvg_canvas_->target(tvg_buffer_.data(), SCREEN_WIDTH, SCREEN_WIDTH, SCREEN_HEIGHT,
...
private:
  ...
  bool nodeContainsPoint(flex::Node *node, float wx, float wy) {
    ...
    if (shape->geometry_type() == flex::GeometryType::Rect) {
      auto r = shape->rect();
      return lx >= nx && lx <= nx + r.width && ly >= ny && ly <= ny + r.height;
    } else if (shape->geometry_type() == flex::GeometryType::Circle) {
      float dx = lx - nx;
      float dy = ly - ny;
      ...
      return lx >= bounds.x && lx <= bounds.x + bounds.width && ly >= bounds.y &&
  ...
  bool nodeIntersectsBox(flex::Node *node, float x1, float y1, float x2, float y2) {
    ...
    if (shape->geometry_type() == flex::GeometryType::Rect) {
      ...
      return !(nx + radius < bx1 || nx - radius > bx2 || ny + radius < by1 || ny - radius > by2);
    }
  ...
  ResizeHandle hitTestHandle(float sx, float sy, flex::Node *&out_node) {
    ...
    for (auto *node : selected_nodes_) {
      ...
      for (int i = 0; i < 4; ++i) {
        if (std::abs(sx - info.corners[i][0]) <= hs && std::abs(sy - info.corners[i][1]) <= hs) {
          out_node = node;
  ...
  bool hitTestRotationHandle(float sx, float sy) {
    ...
    return std::abs(sx - info.rot_handle_x) <= hs && std::abs(sy - info.rot_handle_y) <= hs;
  }

  // Get selection bounds and center for rotation
  bool getSelectionBounds(float &min_x, float &min_y, float &max_x, float &max_y, float &center_x,
                          float &center_y) {
    if (selected_nodes_.empty())
      return false;
  ...
  void beginRotation(float sx, float sy) {
    ...
    if (!getSelectionBounds(min_x, min_y, max_x, max_y, cx, cy))
      return;
  ...
  bool isSelected(flex::Node *node) { return selected_set_.count(node) > 0; }
  ...
  void handleMouseUp(const SDL_MouseButtonEvent &btn) {
    ...
    if (btn.button == SDL_BUTTON_LEFT) {
      ...
      if (selection_box_.active) {
        selectInBox(selection_box_.start_x, selection_box_.start_y, selection_box_.end_x,
                    selection_box_.end_y);
  ...
  void finishDrawing() {
    ...
    std::cout << "Created path: " << shape->id() << " at (" << min_x << ", " << min_y << ") size "
              << w << "x" << h << "\n";
  }
  ...
  void createShape(bool is_circle) {
    ...
    shape->set_position(SCREEN_WIDTH / 2 - camera_.x / camera_.zoom,
                        SCREEN_HEIGHT / 2 - camera_.y / camera_.zoom);
    shape->set_fill(flex::Color(0.3f + (rand() % 50) / 100.0f, 0.3f + (rand() % 50) / 100.0f,
                                0.3f + (rand() % 50) / 100.0f, 1.0f));
  ...
  void render() {
    ...
    if (draw_.active && draw_.points.size() >= 2) {
      ...
      flex_renderer_->stroke_path(preview_path, flex::Paint::solid(preview_color),
                                  pen_settings_.stroke_width);
    }
  ...
  void drawGridTVG() {
    ...
    for (float x = start_x; x <= world_right; x += grid_size) {
      std::string path = "M " + std::to_string(x) + " " + std::to_string(world_top) + " L " +
                         std::to_string(x) + " " + std::to_string(world_bottom);
      flex_renderer_->stroke_path(path, paint, 1.0f / camera_.zoom);
    }

    // Horizontal lines
    for (float y = start_y; y <= world_bottom; y += grid_size) {
      std::string path = "M " + std::to_string(world_left) + " " + std::to_string(y) + " L " +
                         std::to_string(world_right) + " " + std::to_string(y);
      flex_renderer_->stroke_path(path, paint, 1.0f / camera_.zoom);
    }
  ...
  void rotatePoint(float px, float py, float cx, float cy, float angle_deg, float &out_x,
                   float &out_y) {
  ...
  std::string rect_path(float x, float y, float w, float h) {
    return "M " + std::to_string(x) + " " + std::to_string(y) + " H " + std::to_string(x + w) +
           " V " + std::to_string(y + h) + " H " + std::to_string(x) + " Z";
  }

  // Helper: generate SVG line path
  std::string line_path(float x1, float y1, float x2, float y2) {
    return "M " + std::to_string(x1) + " " + std::to_string(y1) + " L " + std::to_string(x2) + " " +
           std::to_string(y2);
  }
  ...
  void drawOverviewTVG() {
    ...
    flex_renderer_->fill_path(rect_path(ox, oy, ow, oh), flex::Paint::solid(rgb(30, 30, 35, 230)));
    flex_renderer_->stroke_path(rect_path(ox, oy, ow, oh), flex::Paint::solid(rgb(60, 60, 70)),
                                1.0f);
    ...
    for (size_t i = 0; i < root->child_count(); ++i) {
      ...
      flex_renderer_->fill_path(rect_path(rx, ry, rw, rh), flex::Paint::solid(col));
    }
    ...
    flex_renderer_->fill_path(rect_path(vx, vy, vw, vh), flex::Paint::solid(rgb(0, 120, 215, 40)));
    // Viewport border
    flex_renderer_->stroke_path(rect_path(vx, vy, vw, vh), flex::Paint::solid(rgb(0, 153, 255)),
                                1.5f);
  }
  ...
  void drawBottomToolbarTVG() {
    ...
    flex_renderer_->stroke_path(
        line_path(bx + 6, by + btn_size / 2, bx + btn_size - 6, by + btn_size / 2),
    ...
    flex_renderer_->draw_text(zoom_text, text_x, by + 17, "Segoe UI", 12.0f, false,
                              rgb(180, 180, 180));
    ...
    // Plus sign
    flex_renderer_->stroke_path(
        line_path(plus_x + 6, by + btn_size / 2, plus_x + btn_size - 6, by + btn_size / 2),
        flex::Paint::solid(rgb(200, 200, 200)), 2.0f);
    flex_renderer_->stroke_path(
        line_path(plus_x + btn_size / 2, by + 6, plus_x + btn_size / 2, by + btn_size - 6),
        flex::Paint::solid(rgb(200, 200, 200)), 2.0f);
  }
  ...
  void drawGizmosTVG() {
    ...
    for (auto *node : selected_nodes_) {
      ...
      std::string frame_path =
          "M " + std::to_string(corners[0][0]) + " " + std::to_string(corners[0][1]) + " L " +
          std::to_string(corners[1][0]) + " " + std::to_string(corners[1][1]) + " L " +
          std::to_string(corners[3][0]) + " " + std::to_string(corners[3][1]) + " L " +
          std::to_string(corners[2][0]) + " " + std::to_string(corners[2][1]) + " Z";
      ...
      for (int i = 0; i < 4; ++i) {
        std::string handle_path =
            rect_path(corners[i][0] - hs, corners[i][1] - hs, handle_size, handle_size);
      ...
      if (first_shape) {
        ...
        flex_renderer_->stroke_path(line_path(edge_mid_x, edge_mid_y, rot_x, rot_y),
                                    flex::Paint::solid(select_color), line_width);

        // Rotation handle square
        float rhs = rot_handle_size / 2;
        std::string rot_path =
            rect_path(rot_x - rhs, rot_y - rhs, rot_handle_size, rot_handle_size);
  ...
  void drawSelectionBoxTVG() {
    ...
    // Border
    flex_renderer_->stroke_path(rect_path(x1, y1, w, h), flex::Paint::solid(rgb(0, 153, 255)),
                                line_width);
  }
  ...
  void drawContextMenuTVG() {
    ...
    // Shadow
    flex_renderer_->fill_path(rect_path(x + 3, y + 3, w, h), flex::Paint::solid(rgb(0, 0, 0, 60)));

    // Background
    flex_renderer_->fill_path(rect_path(x, y, w, h), flex::Paint::solid(rgb(45, 45, 50, 245)));

    // Border
    flex_renderer_->stroke_path(rect_path(x, y, w, h), flex::Paint::solid(rgb(80, 80, 90)), 1.0f);
  ...
  void drawContextMenu() {
    ...
    for (size_t i = 0; i < context_menu_.items.size(); ++i) {
      ...
      if (item.separator) {
        SDL_SetRenderDrawColor(sdl_renderer_, 80, 80, 90, 255);
        SDL_RenderDrawLine(sdl_renderer_, (int)x + 8, (int)cy + 4, (int)(x + w - 8), (int)cy + 4);
        cy += 8;
        continue;
      }
  ...
  void drawToolbarTVG() {
    ...
    // Bottom border
    flex_renderer_->stroke_path(
        line_path(0, Toolbar::HEIGHT - 1, SCREEN_WIDTH, Toolbar::HEIGHT - 1),
        flex::Paint::solid(rgb(60, 60, 70)), 1.0f);
  ...
  void drawToolIcon(ToolType tool, float x, float y, float size, const flex::Color &color) {
    ...
    case ToolType::Select: {
      // Arrow cursor
      std::string arrow = "M " + std::to_string(cx - s) + " " + std::to_string(cy - s) + " L " +
                          std::to_string(cx + s * 0.3f) + " " + std::to_string(cy + s * 0.8f) +
                          " L " + std::to_string(cx) + " " + std::to_string(cy + s * 0.3f) + " L " +
                          std::to_string(cx + s * 0.8f) + " " + std::to_string(cy + s * 0.3f) +
                          " Z";
    ...
    case ToolType::Pan: {
      ...
      flex_renderer_->stroke_path(line_path(cx - s, cy, cx + s, cy), flex::Paint::solid(color),
                                  2.0f);
      flex_renderer_->stroke_path(line_path(cx, cy - s, cx, cy + s), flex::Paint::solid(color),
                                  2.0f);
      // Arrow heads
      flex_renderer_->stroke_path(line_path(cx + s - 3, cy - 3, cx + s, cy),
    ...
    case ToolType::Circle: {
      // Circle
      std::string circle = "M " + std::to_string(cx + s) + " " + std::to_string(cy) + " A " +
                           std::to_string(s) + " " + std::to_string(s) + " 0 1 1 " +
                           std::to_string(cx - s) + " " + std::to_string(cy) + " A " +
                           std::to_string(s) + " " + std::to_string(s) + " 0 1 1 " +
    ...
    case ToolType::Pen: {
      ...
      std::string wavy = "M " + std::to_string(cx - s) + " " + std::to_string(cy) + " Q " +
                         std::to_string(cx - s * 0.5f) + " " + std::to_string(cy - s * 0.5f) + " " +
                         std::to_string(cx) + " " + std::to_string(cy) + " Q " +
                         std::to_string(cx + s * 0.5f) + " " + std::to_string(cy + s * 0.5f) + " " +
                         std::to_string(cx + s) + " " + std::to_string(cy);
  ...
  void drawCommandIcon(CommandType cmd, float x, float y, float size, const flex::Color &color) {
    ...
    case CommandType::Group: {
      ...
      std::string left = "M " + std::to_string(cx - s * 0.3f) + " " + std::to_string(cy - s) +
                         " L " + std::to_string(cx - s) + " " + std::to_string(cy - s) + " L " +
                         std::to_string(cx - s) + " " + std::to_string(cy + s) + " L " +
                         std::to_string(cx - s * 0.3f) + " " + std::to_string(cy + s);
    ...
    case CommandType::Ungroup: {
      // Exploding brackets
      std::string left =
          "M " + std::to_string(cx - s * 0.5f) + " " + std::to_string(cy - s * 0.8f) + " L " +
          std::to_string(cx - s * 1.2f) + " " + std::to_string(cy - s * 0.8f) + " L " +
          std::to_string(cx - s * 1.2f) + " " + std::to_string(cy + s * 0.8f) + " L " +
          std::to_string(cx - s * 0.5f) + " " + std::to_string(cy + s * 0.8f);
      std::string right =
          "M " + std::to_string(cx + s * 0.5f) + " " + std::to_string(cy - s * 0.8f) + " L " +
          std::to_string(cx + s * 1.2f) + " " + std::to_string(cy - s * 0.8f) + " L " +
          std::to_string(cx + s * 1.2f) + " " + std::to_string(cy + s * 0.8f) + " L " +
    ...
    case CommandType::BringFront: {
      // Up arrow
      std::string arrow = "M " + std::to_string(cx) + " " + std::to_string(cy - s) + " L " +
                          std::to_string(cx - s) + " " + std::to_string(cy + s * 0.5f) + " L " +
                          std::to_string(cx + s) + " " + std::to_string(cy + s * 0.5f) + " Z";
      flex_renderer_->fill_path(arrow, flex::Paint::solid(color));
      break;
    }
  ...
  void drawPanelTVG() {
    ...
    // Panel background
    flex_renderer_->fill_path(rect_path(px, py, pw, ph), flex::Paint::solid(rgb(35, 35, 40, 245)));

    // Left border
    flex_renderer_->stroke_path(line_path(px, py, px, py + ph), flex::Paint::solid(rgb(60, 60, 70)),
                                1.0f);
    ...
    flex_renderer_->draw_text(header_text, px + 10, py + 18, "Segoe UI", 12.0f, true,
                              rgb(180, 180, 180));
    ...
    for (const auto &section : panel_.sections) {
      ...
      if (section.collapsed) {
        tri_path = "M " + std::to_string(tri_x) + " " + std::to_string(tri_y) + " L " +
                   std::to_string(tri_x + 6) + " " + std::to_string(tri_y + 4) + " L " +
                   std::to_string(tri_x) + " " + std::to_string(tri_y + 8) + " Z";
      } else {
        tri_path = "M " + std::to_string(tri_x) + " " + std::to_string(tri_y) + " L " +
                   std::to_string(tri_x + 8) + " " + std::to_string(tri_y) + " L " +
                   std::to_string(tri_x + 4) + " " + std::to_string(tri_y + 6) + " Z";
      }
      flex_renderer_->fill_path(tri_path, flex::Paint::solid(rgb(150, 150, 150)));

      // Section title
      flex_renderer_->draw_text(section.title, px + 20, sy + 16, "Segoe UI", 11.0f, false,
                                rgb(180, 180, 180));
  ...
  void drawTransformProperties(float x, float y, float w) {
    if (selected_nodes_.empty()) {
      flex_renderer_->draw_text("No selection", x, y + 12, "Segoe UI", 10.0f, false,
                                rgb(100, 100, 100));
      return;
    }
    ...
    flex_renderer_->draw_text(std::to_string((int)node->x()), x + 30, y + 12, "Segoe UI", 10.0f,
                              false, rgb(200, 200, 200));

    // Y position
    flex_renderer_->draw_text("Y", x + w / 2, y + 12, "Segoe UI", 10.0f, false, rgb(140, 140, 140));
    flex_renderer_->draw_text(std::to_string((int)node->y()), x + w / 2 + 30, y + 12, "Segoe UI",
                              10.0f, false, rgb(200, 200, 200));
    ...
    if (shape) {
      if (shape->geometry_type() == flex::GeometryType::Rect) {
        ...
        flex_renderer_->draw_text(std::to_string((int)r.width), x + 30, y + 12, "Segoe UI", 10.0f,
                                  false, rgb(200, 200, 200));
        flex_renderer_->draw_text("H", x + w / 2, y + 12, "Segoe UI", 10.0f, false,
                                  rgb(140, 140, 140));
        flex_renderer_->draw_text(std::to_string((int)r.height), x + w / 2 + 30, y + 12, "Segoe UI",
                                  10.0f, false, rgb(200, 200, 200));
      } else if (shape->geometry_type() == flex::GeometryType::Circle) {
        auto c = shape->circle();
        flex_renderer_->draw_text("R", x, y + 12, "Segoe UI", 10.0f, false, rgb(140, 140, 140));
        flex_renderer_->draw_text(std::to_string((int)c.radius), x + 30, y + 12, "Segoe UI", 10.0f,
                                  false, rgb(200, 200, 200));
      }
    }
    ...
    flex_renderer_->draw_text(std::to_string((int)node->rotation()) + "°", x + 30, y + 12,
                              "Segoe UI", 10.0f, false, rgb(200, 200, 200));
  }
  ...
  void drawAppearanceProperties(float x, float y, float w) {
    ...
    if (shape->has_fill()) {
      auto f = shape->fill();
      // Draw color swatch
      flex_renderer_->fill_path(rect_path(x + 40, y + 2, 60, 16), flex::Paint::solid(f.color));
    ...
    if (shape->has_stroke()) {
      auto s = shape->stroke();
      flex_renderer_->fill_path(rect_path(x + 40, y + 2, 60, 16), flex::Paint::solid(s.color));
      flex_renderer_->stroke_path(rect_path(x + 40, y + 2, 60, 16),
                                  flex::Paint::solid(rgb(80, 80, 80)), 1.0f);
    }
  }
  ...
  void drawPenSettings(float x, float y, float w) {
    ...
    for (int i = 0; i < PenSettings::NUM_COLORS; ++i) {
      ...
      bool is_selected =
          (col.r == pen_settings_.stroke_color.r && col.g == pen_settings_.stroke_color.g &&
    ...
    for (float ww : widths) {
      ...
      flex_renderer_->stroke_path(rect_path(cx, y, 30, 20), flex::Paint::solid(rgb(80, 80, 80)),
                                  1.0f);
      flex_renderer_->draw_text(std::to_string((int)ww), cx + 10, y + 14, "Segoe UI", 10.0f, false,
                                rgb(200, 200, 200));
      cx += 34;
    }
    ...
    flex_renderer_->stroke_path(rect_path(x + 70, y, 70, 20), flex::Paint::solid(rgb(80, 80, 80)),
                                1.0f);
    flex_renderer_->draw_text("Smooth", x + 82, y + 14, "Segoe UI", 10.0f, false,
                              rgb(200, 200, 200));
  }
  ...
  bool handlePenSettingsClick(float local_x, float local_y) {
    ...
    for (int i = 0; i < PenSettings::NUM_COLORS; ++i) {
      ...
      if (local_x >= cx && local_x < cx + swatch_size && content_y >= cy &&
          content_y < cy + swatch_size) {
    ...
    float wx = x + 50;
    for (int i = 0; i < 4; ++i) {
      if (local_x >= wx && local_x < wx + 30 && content_y >= width_y && content_y < width_y + 20) {
    ...
    // Check fill toggle
    if (local_x >= x && local_x < x + 60 && content_y >= toggle_y && content_y < toggle_y + 20) {
    ...
    // Check smooth toggle
    if (local_x >= x + 70 && local_x < x + 140 && content_y >= toggle_y &&
        content_y < toggle_y + 20) {
  ...
  void pasteClipboard() {
    ...
    for (const auto &item : clipboard_) {
      ...
      shape->set_position(SCREEN_WIDTH / 2 - camera_.x / camera_.zoom + offset_x,
