...
private:
    ...
    void setup_layouts() {
        ...
        if (auto* toolbar = dynamic_cast<flex::Group*>(
                artboard->find("scene1")->find("toolbar"))) {
        ...
        if (auto* menu = dynamic_cast<flex::Group*>(
                artboard->find("scene2")->find("menu"))) {
        ...
        if (auto* centerContainer = dynamic_cast<flex::Group*>(
                artboard->find("scene3")->find("centerContainer"))) {
        ...
        if (auto* spaceBetween = dynamic_cast<flex::Group*>(
                artboard->find("scene4")->find("spaceBetweenContainer"))) {
        ...
        if (auto* outerColumn = dynamic_cast<flex::Group*>(
                artboard->find("scene5")->find("outerColumn"))) {
            // Outer column
        ...
        // Scene 6: Padding & Gap
        if (auto* paddedContainer = dynamic_cast<flex::Group*>(
                artboard->find("scene6")->find("paddedContainer"))) {
