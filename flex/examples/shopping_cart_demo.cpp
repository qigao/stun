...
public:
    ...
    void clear() {
        for (auto& p : products_) p.quantity = 0;
    }

    float get_subtotal() const {
        float s = 0;
        for (const auto& p : products_) s += p.subtotal();
        return s;
    }
    ...
    int get_total_items() const {
        int count = 0;
        for (const auto& p : products_) count += p.quantity;
        return count;
    }
...
public:
    ...
    ShoppingCartView(flex::Instance::Ptr instance) : instance_(instance) {
        ...
        // Initialize child nodes from artboard
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            auto* item_row = artboard->find(id);
            if (item_row) {
        ...
        // Summary nodes
        subtotal_text_ = artboard->find("subtotalValue");
        tax_text_      = artboard->find("taxValue");
        total_text_    = artboard->find("totalValue");
...
public:
    bool init() {
        ...
        window_ = SDL_CreateWindow(
            "Shopping Cart - Flex Engine MVC Demo",
