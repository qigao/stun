...
void register_button(const WidgetTheme &theme) {
  ...
  button->set_builder([theme](const Props &props) -> Node::Ptr {
    ...
    // Background
    auto bg = create_rect_bg(width, height, bg_color, theme.corner_radius, &border_color,
                             theme.border_width);
    bg->set_id("bg");
    group->add_child(bg);
    ...
    label->set_position(width / 2 - text_str.length() * theme.font_size * 0.3f,
                        height / 2 - theme.font_size / 2);
...
void register_checkbox(const WidgetTheme &theme) {
  ...
  checkbox->set_builder([theme](const Props &props) -> Node::Ptr {
    ...
    auto box =
        create_rect_bg(box_size, box_size, bg_color, 3.0f, &border_color, theme.border_width);
    box->set_id("box");
    group->add_child(box);
...
void register_toggle(const WidgetTheme &theme) {
  ...
  toggle->set_builder([theme](const Props &props) -> Node::Ptr {
    ...
    auto track =
        create_rect_bg(width, height, track_color, height / 2, &border_color, theme.border_width);
    track->set_id("track");
...
void register_combobox(const WidgetTheme &theme) {
  ...
  combo->set_builder([theme](const Props &props) -> Node::Ptr {
    ...
    std::string selected_text =
        (selected_idx >= 0 && selected_idx < (int)items.size()) ? items[selected_idx] : "";
    ThemeColor text_color = enabled ? theme.text_primary : theme.text_disabled;
    ...
    arrow->set_stroke(Color(text_color.r() / 255.0f, text_color.g() / 255.0f,
                            text_color.b() / 255.0f, text_color.a() / 255.0f),
                      1.5f);
...
void register_listbox(const WidgetTheme &theme) {
  ...
  listbox->set_builder([theme](const Props &props) -> Node::Ptr {
    ...
    // Background
    auto bg = create_rect_bg(width, height, theme.surface, theme.corner_radius, &theme.border,
                             theme.border_width);
    bg->set_id("bg");
    group->add_child(bg);
    ...
    for (size_t i = 0; i < items.size(); ++i) {
      ...
      ThemeColor text_color =
          (static_cast<int>(i) == selected_idx) ? theme.accent : theme.text_primary;
