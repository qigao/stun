...
inline Shape::Ptr create_rect_bg(
    float width, float height,
    const ThemeColor& fill,
    float corner_radius = 0,
    const ThemeColor* stroke = nullptr,
    float stroke_width = 1.0f
) {
    auto shape = Shape::create();
    shape->set_rect(width, height, corner_radius);
    shape->set_fill(Color(fill.r() / 255.0f, fill.g() / 255.0f, fill.b() / 255.0f, fill.a() / 255.0f));
    if (stroke) {
        shape->set_stroke(Color(stroke->r() / 255.0f, stroke->g() / 255.0f, stroke->b() / 255.0f, stroke->a() / 255.0f), stroke_width);
    }
    return shape;
}

// Create text label
inline Text::Ptr create_text(
    ...
    text->set_color(Color(color.r() / 255.0f, color.g() / 255.0f, color.b() / 255.0f, color.a() / 255.0f));
...
// Create circle
inline Shape::Ptr create_circle(
    float radius,
    const ThemeColor& fill,
    const ThemeColor* stroke = nullptr,
    float stroke_width = 1.0f
...
inline std::string get_string(const Props& props, const std::string& key, const std::string& default_val = "") {
    auto it = props.find(key);
    if (it != props.end()) {
...
inline void apply_state_style(
    ...
    if (border_shape) {
        border_shape->set_stroke(Color(
            border_color.r() / 255.0f,
            border_color.g() / 255.0f,
            border_color.b() / 255.0f,
            border_color.a() / 255.0f
        ));
    }
}
