...
Component::Component(const std::string &name) : name_(name), builder_(nullptr) {}

void Component::add_prop(const std::string &name, PropValue default_value,
                         const std::string &desc) {
  prop_defs_[name] = PropDef{name, default_value, desc};
}

void Component::add_prop(const PropDef &prop) { prop_defs_[prop.name] = prop; }
...
const PropDef *Component::get_prop_def(const std::string &name) const {
  ...
  if (it == prop_defs_.end())
    return nullptr;
  return &it->second;
}

ComponentNodePtr Component::instantiate(const Props &props) const {
  if (!builder_) {
...
bool Component::validate_props(const Props &props, std::string &error) const {
  ...
  for (const auto &[name, value] : props) {
    const PropDef* def = get_prop_def(name);
    if (!def) {
      error = "Unknown prop: " + name;
      return false;
    }

    if (def->default_value.index() != value.index()) {
      auto type_name = [](const PropValue& v) -> const char* {
        ...
        if (std::holds_alternative<uint32_t>(v)) return "color";
        return "unknown";
      };
      error = "Type mismatch for prop: " + name + " (expected " +
              type_name(def->default_value) + ", got " + type_name(value) + ")";
      return false;
    }
  }
...
void ComponentRegistry::register_component(Component::SharedPtr component) {
  if (!component)
    return;
...
void ComponentRegistry::register_component(const std::string &name,
                                           Component::BuilderFunc builder) {
  auto component = Component::create(name);
  component->set_builder(builder);
  register_component(component);
}

Component::SharedPtr ComponentRegistry::get(const std::string &name) const {
...
void ComponentRegistry::clear() { components_.clear(); }
