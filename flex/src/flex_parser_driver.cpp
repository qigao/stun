...
public:
    ...
    void on_token(const Token& tok, const Token* prev_tok) {
        ...
        case TOK_EQ:
        case TOK_NEQ:
            if (tok.type == TOK_GT) current_transition_.condition_op = ">";
            else if (tok.type == TOK_LT) current_transition_.condition_op = "<";
            else if (tok.type == TOK_EQ) current_transition_.condition_op = "==";
            else if (tok.type == TOK_NEQ) current_transition_.condition_op = "!=";
...
private:
    ...
    void handle_lbrace(const Token& tok) {
        ...
        else if (in_data_item_ == false && data_brace_depth_ > 0 && current_data_item_.key.length() > 0) {
            in_data_item_ = true;
    ...
    void substitute_for_item(std::shared_ptr<AstNode>& node,
                             const std::string& iterator,
                             const AstDataItem& item,
                             int index) {
        ...
        for (auto& [key, value] : node->properties) {
            if (auto* sval = std::get_if<std::string>(&value)) {
                ...
                while ((pos = result.find(prefix, start)) != std::string::npos) {
                    ...
                    std::string prop_name = result.substr(pos + prefix.length(),
    ...
    void set_property(const AstValue& value) {
        if (pending_prop_key_.empty()) return;
...
static std::shared_ptr<Node> convert_ast_node(const std::shared_ptr<AstNode>& ast_node) {
    ...
    if (ast_node->type == "group") {
        ...
        if (component) {
            ...
            static const std::set<std::string> node_props = {
                "x", "y", "width", "height", "opacity", "rotation", "visible", "id"
            };

            Props props;
            for (const auto& [key, ast_value] : ast_node->properties) {
                // Skip Node-level properties - they're applied after instantiation
                if (node_props.count(key)) continue;
    ...
    for (const auto& [key, value] : ast_node->properties) {
        if (key == "x") {
            ...
            if (auto sval = std::get_if<std::string>(&value)) {
                if (*sval == "auto") node->set_align_self(AlignSelf::Auto);
                else if (*sval == "start") node->set_align_self(AlignSelf::Start);
                else if (*sval == "end") node->set_align_self(AlignSelf::End);
                else if (*sval == "center") node->set_align_self(AlignSelf::Center);
                else if (*sval == "stretch") node->set_align_self(AlignSelf::Stretch);
            }
        }
            ...
            if (key == "width") {
                ...
                if (auto fval = std::get_if<float>(&value)) {
                    if (ast_node->type == "star") {
                        auto star_geom = shape->star();
                        shape->set_star(static_cast<int>(*fval), star_geom.outer_radius, star_geom.inner_radius);
                    }
                }
            } else if (key == "rx") {
                if (auto fval = std::get_if<float>(&value)) {
        ...
        if (auto group = std::dynamic_pointer_cast<Group>(node)) {
            if (key == "layout") {
                ...
                if (auto sval = std::get_if<std::string>(&value)) {
                    if (*sval == "row") group->set_flex_direction(FlexDirection::Row);
                    else if (*sval == "column") group->set_flex_direction(FlexDirection::Column);
                    else if (*sval == "rowReverse") group->set_flex_direction(FlexDirection::RowReverse);
                    else if (*sval == "columnReverse") group->set_flex_direction(FlexDirection::ColumnReverse);
                }
            } else if (key == "justifyContent") {
                if (auto sval = std::get_if<std::string>(&value)) {
                    if (*sval == "start") group->set_justify_content(JustifyContent::Start);
                    else if (*sval == "end") group->set_justify_content(JustifyContent::End);
                    else if (*sval == "center") group->set_justify_content(JustifyContent::Center);
                    else if (*sval == "spaceBetween") group->set_justify_content(JustifyContent::SpaceBetween);
                    else if (*sval == "spaceAround") group->set_justify_content(JustifyContent::SpaceAround);
                    else if (*sval == "spaceEvenly") group->set_justify_content(JustifyContent::SpaceEvenly);
                }
            } else if (key == "alignItems") {
                if (auto sval = std::get_if<std::string>(&value)) {
                    if (*sval == "start") group->set_align_items(AlignItems::Start);
                    else if (*sval == "end") group->set_align_items(AlignItems::End);
                    else if (*sval == "center") group->set_align_items(AlignItems::Center);
                    else if (*sval == "stretch") group->set_align_items(AlignItems::Stretch);
                }
            } else if (key == "flexWrap") {
                if (auto sval = std::get_if<std::string>(&value)) {
                    if (*sval == "nowrap" || *sval == "noWrap") group->set_flex_wrap(FlexWrap::NoWrap);
                    else if (*sval == "wrap") group->set_flex_wrap(FlexWrap::Wrap);
                }
            } else if (key == "gap") {
                if (auto fval = std::get_if<float>(&value)) {
                    group->set_gap(*fval);
                ...
                if (auto fval = std::get_if<float>(&value)) {
                    group->set_padding(*fval, group->padding_right(), group->padding_bottom(), group->padding_left());
                }
            } else if (key == "paddingRight") {
                if (auto fval = std::get_if<float>(&value)) {
                    group->set_padding(group->padding_top(), *fval, group->padding_bottom(), group->padding_left());
                }
            } else if (key == "paddingBottom") {
                if (auto fval = std::get_if<float>(&value)) {
                    group->set_padding(group->padding_top(), group->padding_right(), *fval, group->padding_left());
                }
            } else if (key == "paddingLeft") {
                if (auto fval = std::get_if<float>(&value)) {
                    group->set_padding(group->padding_top(), group->padding_right(), group->padding_bottom(), *fval);
                }
            }
        }
    }
