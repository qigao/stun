/*
 * Flex Engine - NanoVG Backend Renderer Implementation
 *
 * Immediate-mode backend backed by an externally owned NVGcontext.
...
public:
  ...
  bool register_font(const std::string& family,
    ...
    return nanovg_backend::load_font(family.c_str(), path.c_str());
  }

  void unregister_font(const std::string& family) override {
    nanovg_backend::unload_font(family.c_str());
  }
...
private:
  ...
  RasterImage ensure_svg_image(const std::string &key, const lunasvg::Document &document, float width,
                               float height) {
    ...
    const int handle =
        nvgCreateImageRGBA(vg_, bitmap.width(), bitmap.height(), 0, bitmap.data());
    if (handle <= 0) {
      return {};
    }
  ...
  std::string resolve_font_name(const std::string &requested, bool bold) {
    ...
    std::string fallback;
    {
      std::lock_guard<std::mutex> lock(nanovg_backend::font_registry_mutex());
      auto &registry = nanovg_backend::font_registry();
      if (!registry.empty()) {
...
std::unique_ptr<Renderer> create_nanovg_renderer(CanvasHandle canvas) {
  auto *vg = static_cast<NVGcontext *>(canvas);
  if (!vg) {
    return nullptr;
  }
