/*
 * Flex Engine - ThorVG Backend Renderer Implementation
 *
...
public:
  ...
  bool register_font(const std::string& family,
    ...
    return thorvg_backend::load_font(family.c_str(), path.c_str());
  }

  void unregister_font(const std::string& family) override {
    thorvg_backend::unload_font(family.c_str());
  }
  ...
  void set_retained_insertion_anchor(PaintHandle before) override {
    retained_insertion_anchor_ = static_cast<tvg::Paint *>(before);
  }
  ...
  void end_frame() override {
    ...
    // Retained paints survive between frames, but a swapped GL back buffer does
    // not. Clear and redraw the retained scene so partial UI updates cannot
    // expose stale or empty pixels after buffer swaps.
    canvas_->draw(true);
    ...
    if (total_ms > 32.0 || frame_count_ % 100 == 0) {
      TLOG_INFO("[ThorVG] Frame {} | remove: {:.1f}ms | push({}): {:.1f}ms | draw: {:.1f}ms | sync: {:.1f}ms | total: {:.1f}ms",
           frame_count_, remove_ms, push_count_, push_ms, draw_ms, sync_ms, total_ms);
      if (detail_profile_enabled()) {
        TLOG_INFO("[ThorVG]   replay detail | shape({}): {:.1f}ms | text({}): {:.1f}ms | image({}): {:.1f}ms | svg({}): {:.1f}ms | canvas_push: {:.1f}ms | paths:{} clips:{}",
             shape_count_, shape_ms_, text_count_, text_ms_, image_count_, image_ms_,
             svg_count_, svg_ms_, push_only_ms_, path_parse_count_, clip_count_);
      }
    }
  }
  ...
  void rotate(float degrees) override {
    ...
    float c = std::cos(rad);
    float s = std::sin(rad);
    float m00 = current_transform_.data[0], m01 = current_transform_.data[1];
    float m10 = current_transform_.data[3], m11 = current_transform_.data[4];
  ...
  void scale(float sx, float sy) override {
    current_transform_.data[0] *= sx; current_transform_.data[1] *= sy;
    current_transform_.data[3] *= sx; current_transform_.data[4] *= sy;
  }

  // Clipping
  void clip_rect(float x, float y, float w, float h) override {
    current_clip_ = {x, y, w, h, current_transform_, true};
  }
  ...
  void fill_path(const std::string &d, const Paint &paint) override {
    auto start = detail_profile_start();
    ...
    shape_count_++;
    add_detail_time(shape_ms_, start);
  }
  ...
  void draw_text(const std::string &text, float x, float y, const std::string &font_family,
    ...
    const std::string resolved_font = font_family.empty() ? "Arial" : font_family;
    tvg_text->font(resolved_font.c_str());
    ...
    text_count_++;
    add_detail_time(text_ms_, start);
    (void)bold;
  }
  ...
  void draw_image(const std::string &src, float x, float y, float width, float height) override {
    ...
    image_count_++;
    add_detail_time(image_ms_, start);
  }
  ...
  void clear(const Color &color) override {
    auto start = detail_profile_start();
    if (retained_mode_ && background_shape_ && background_width_ == width_ &&
        background_height_ == height_) {
      if (background_color_ != color) {
        background_shape_->fill(to_u8(color.r), to_u8(color.g), to_u8(color.b),
                                to_u8(color.a));
    ...
    if (retained_mode_ && background_shape_) {
      canvas_->remove(background_shape_);
      background_shape_ = nullptr;
    }
  ...
  RendererCapabilities capabilities() const override {
    ...
    // ThorVG paint handles cannot currently survive arbitrary slot removal and
    // reinsertion during flexUI subtree replacement. Use full-frame replay until
    // the retained cache has a backend-level ordering contract.
  ...
  PaintHandle push_rect(float x, float y, float w, float h, float r, const Paint &fill,
    ...
    return push_retained_to_canvas(shape);
  }
  ...
  PaintHandle push_text(const std::string &text, const std::string &font_family,
                        float font_size, bool bold, const Color &color,
                        const Transform &transform, float alpha) override {
    if (text.empty() || font_size <= 0.0f || color.a <= 0.0f)
      return retained_null();
    auto tvg_text = tvg::Text::gen();
    const std::string resolved_font = font_family.empty() ? "Arial" : font_family;
    tvg_text->font(resolved_font.c_str());
    tvg_text->size(font_size);
    tvg_text->text(text.c_str());
    tvg_text->transform(to_tvg_matrix(transform));
    tvg_text->fill(to_u8(color.r), to_u8(color.g), to_u8(color.b));
    tvg_text->opacity(static_cast<uint8_t>(color.a * alpha * 255));
    apply_clip(tvg_text);
    text_count_++;
    (void)bold;
    return push_retained_to_canvas(tvg_text);
  }
  ...
  PaintHandle push_image(const std::string &src, float width, float height,
    ...
    if (picture->load(src.c_str()) != tvg::Result::Success)
      return retained_null();
    picture->size(width, height);
    picture->transform(to_tvg_matrix(transform));
    picture->opacity(static_cast<uint8_t>(alpha * 255));
    apply_clip(picture);
    image_count_++;
    return push_retained_to_canvas(picture);
  }
...
private:
  ...
  Transform current_transform_ = Transform{};
  Shadow current_shadow_;
  BlurFilter current_blur_;
  tvg::Shape *background_shape_ = nullptr; // Cached background for retained mode
  tvg::Paint *retained_insertion_anchor_ = nullptr;
  float background_width_ = 0.0f;
  float background_height_ = 0.0f;
  ...
  int image_count_ = 0;
  int svg_count_ = 0;
  double shape_ms_ = 0.0;
  double text_ms_ = 0.0;
  double image_ms_ = 0.0;
  double svg_ms_ = 0.0;
  double push_only_ms_ = 0.0;
  ...
  static bool detail_profile_enabled() {
    static const bool enabled = [] {
      const char* value = std::getenv("FLEX_THORVG_DETAIL_PROFILE");
      if (!value || value[0] == '\0') {
        return false;
      }
  ...
  static std::chrono::high_resolution_clock::time_point detail_profile_start() {
    return detail_profile_enabled() ? std::chrono::high_resolution_clock::now()
                                    : std::chrono::high_resolution_clock::time_point{};
  }

  static void add_detail_time(double &target,
  ...
  template <typename T> void push_to_canvas(T &paint) {
    ...
    add_detail_time(push_only_ms_, start);
  }

  PaintHandle retained_null() {
    retained_insertion_anchor_ = nullptr;
    return nullptr;
  }

  template <typename T> PaintHandle push_retained_to_canvas(T &paint) {
    auto start = detail_profile_start();
    auto *raw_ptr = static_cast<tvg::Paint *>(paint);
    push_count_++;
    canvas_->push(std::move(paint), retained_insertion_anchor_);
    retained_insertion_anchor_ = nullptr;
    add_detail_time(push_only_ms_, start);
  ...
  static tvg::Matrix to_tvg_matrix(const Transform& t) {
    tvg::Matrix m;
    m.e11 = t.data[0]; m.e12 = t.data[1]; m.e13 = t.data[2];
    m.e21 = t.data[3]; m.e22 = t.data[4]; m.e23 = t.data[5];
  ...
  tvg::Matrix build_matrix_with_offset(float x, float y) {
    ...
    if (is_translation_only(current_transform_)) {
      tvg::Matrix m;
      m.e11 = 1.0f; m.e12 = 0.0f; m.e13 = x + tx(current_transform_);
      m.e21 = 0.0f; m.e22 = 1.0f; m.e23 = y + ty(current_transform_);
      m.e31 = 0.0f; m.e32 = 0.0f; m.e33 = 1.0f;
      return m;
    }
    ...
    tvg::Matrix m;
    m.e11 = current_transform_.data[0]; m.e12 = current_transform_.data[1]; m.e13 = world_pos.x;
    m.e21 = current_transform_.data[3]; m.e22 = current_transform_.data[4]; m.e23 = world_pos.y;
    m.e31 = 0.0f; m.e32 = 0.0f; m.e33 = 1.0f;
    return m;
  }
  ...
  void apply_clip(tvg::Paint *paint) {
    ...
    clipper->fill(255, 255, 255);
    if (!is_identity(current_clip_.transform)) {
      clipper->transform(to_tvg_matrix(current_clip_.transform));
    }
  ...
  void apply_matrix(tvg::Paint *paint, const Transform &transform) {
    // Skip if identity - no transform needed
    if (is_identity(transform)) return;
