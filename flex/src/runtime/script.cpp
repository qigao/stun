...
struct ScriptContext::Impl {
  ...
  IInstanceContext *bound_instance = nullptr;
...
static JSValue native_func_callback(JSContext *ctx, JSValueConst this_val, int argc,
...
ScriptValue ScriptContext::call(const std::string &func_name,
  ...
  // Call function
  JSValue result =
      JS_Call(impl_->ctx, func, global, static_cast<int>(js_args.size()), js_args.data());
...
void ScriptContext::bind_function(const std::string &name, NativeFunc func) {
  ...
  JSValue js_func = JS_NewCFunctionMagic(impl_->ctx, native_func_callback, name.c_str(), 0,
                                         JS_CFUNC_generic_magic, index);
...
void ScriptContext::set_instance(IInstanceContext *instance) { impl_->bound_instance = instance; }

IInstanceContext *ScriptContext::get_instance() const { return impl_->bound_instance; }

void ScriptContext::clear_instance() { impl_->bound_instance = nullptr; }
...
void ScriptEngine::bind_flex_api(ScriptContext *ctx, IInstanceContext *instance) {
...
void bind_nodes(ScriptContext *ctx, IInstanceContext * /*instance*/) {
...
void bind_events(ScriptContext *ctx, IInstanceContext * /*instance*/) {
  // flex.sendEvent(name)
...
void bind_animations(ScriptContext *ctx, IInstanceContext * /*instance*/) {
...
void bind_machine(ScriptContext *ctx, IInstanceContext * /*instance*/) {
