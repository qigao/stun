...
public:
  ...
  void end_frame() override {
    ...
    static double total_draw_ms = 0;
    static double total_sync_ms = 0;
    ...
    // Accumulate timings
    double draw_ms = std::chrono::duration<double, std::milli>(t_draw - t_start).count();
    double sync_ms = std::chrono::duration<double, std::milli>(t_sync - t_draw).count();
  ...
  void translate(float x, float y) override {
    // Translation must be affected by current rotation and scale
    float rad = current_transform_.rotation * static_cast<float>(M_PI) / 180.0f;
    ...
    current_transform_.tx += rx * current_transform_.sx;
    current_transform_.ty += ry * current_transform_.sy;
  }

  void rotate(float degrees) override { current_transform_.rotation += degrees; }
  ...
  void fill_path(const std::string &d, const Paint &paint) override {
    ...
    // Fast path: detect common geometries and use native ThorVG API
    if (!try_fast_geometry(shape, d)) {
      // Fallback: parse SVG path string
  ...
  bool try_fast_geometry(tvg::Shape *shape, const std::string &d) {
    ...
    if (d[0] == 'M' && d.find('A') != std::string::npos) {
      ...
      if (num_count == 1 && r > 0) {
        ...
        if (d == buf) {
          // Perfect circle match - use native API
          shape->appendCircle(0, 0, r, r); // ThorVG: cx, cy, rx, ry
          fast_hits++;
          return true;
        }
      }
    ...
    if (d[0] == 'M' && d.find('A') != std::string::npos) {
      ...
      if (num_count == 2 && rx > 0 && ry > 0 && rx != ry) {
        ...
        if (d == buf) {
          // Perfect ellipse match - ThorVG appendCircle is actually appendEllipse!
          shape->appendCircle(0, 0, rx, ry); // cx, cy, rx, ry
          fast_hits++;
          return true;
        }
      }
