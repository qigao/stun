...
public:
  ...
  void begin_frame(float width, float height, float pixel_ratio) override {
    ...
    canvas_->remove();
    background_shape_ = nullptr; // Reset background cache
    ...
    clip_count_ = 0;
    path_parse_count_ = 0;
    retained_mode_ = false; // Immediate mode: canvas was cleared
  }
  ...
  void begin_retained_frame(float width, float height, float pixel_ratio) override {
    frame_start_ = std::chrono::high_resolution_clock::now();
  ...
  void remove_cached(tvg::Paint *paint) override {
    if (paint) {
      canvas_->remove(paint);
    }
  }

  tvg::Shape *push_rect(float x, float y, float w, float h, float r, const Paint &fill,
  ...
  tvg::Shape *push_circle(float cx, float cy, float r, const Paint &fill, const Paint &stroke,
  ...
  tvg::Shape *push_ellipse(float cx, float cy, float rx, float ry, const Paint &fill,
                           const Paint &stroke, float stroke_width, const Transform &transform,
                           float alpha) override {
    auto shape = tvg::Shape::gen();
    shape->appendCircle(cx, cy, rx, ry);
  ...
  tvg::Shape *push_polygon(int sides, float radius, const Paint &fill, const Paint &stroke,
                           float stroke_width, const Transform &transform, float alpha) override {
    if (sides < 3 || radius <= 0)
      return nullptr;
  ...
  tvg::Shape *push_star(int points, float outer_radius, float inner_radius, const Paint &fill,
                        const Paint &stroke, float stroke_width, const Transform &transform,
                        float alpha) override {
  ...
  tvg::Shape *push_path(const std::string &d, const Paint &fill, const Paint &stroke,
                        float stroke_width, const Transform &transform, float alpha) override {
    if (d.empty())
      return nullptr;
  ...
  void update_transform(tvg::Paint *paint, const Transform &transform) override {
    if (!paint)
      return;
    apply_matrix(paint, transform);
    update_count_++;
  }
...
private:
  ...
  tvg::Matrix build_matrix() {
    tvg::Matrix m;
    m.e11 = current_transform_.m[0];
    m.e12 = current_transform_.m[1];
    m.e13 = current_transform_.m[2];
    m.e21 = current_transform_.m[3];
    m.e22 = current_transform_.m[4];
    m.e23 = current_transform_.m[5];
    m.e31 = 0.0f;
    m.e32 = 0.0f;
    m.e33 = 1.0f;
    return m;
  ...
  void apply_transform(tvg::Shape *shape) { shape->transform(build_matrix()); }
