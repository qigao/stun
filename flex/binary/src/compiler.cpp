...
std::vector<uint8_t> BinaryCompiler::compile_source(const char* source) {
    ...
    if (!definition->input_schema().empty()) {
        error_message_ =
            "Binary format does not yet support runtime var schemas; compile from source";
        return {};
    }

    if (!definition->impl_->bindings.empty()) {
        error_message_ =
            "Binary format does not yet support property bindings; compile from source";
        return {};
    }

    if (!definition->impl_->component_bindings.empty()) {
        error_message_ =
            "Binary format does not yet support component bindings; compile from source";
        return {};
    }

    if (!definition->impl_->machines.empty()) {
        error_message_ =
            "Binary format does not yet support state machines; compile from source";
        return {};
    }

    if (!definition->impl_->parsed_assets.empty()) {
        error_message_ =
            "Binary format does not yet support asset manifests; compile from source";
        return {};
    }
...
bool BinaryCompiler::compile_to_file(const char* input_path, const char* output_path) {
    ...
    if (!output_path || !output_path[0]) {
        error_message_ = "Empty output path";
        return false;
    }
