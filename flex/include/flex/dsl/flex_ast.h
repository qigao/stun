...
struct AstTransition {
  ...
  std::string to_state;
  std::string condition_expr; // Expression. Empty = unconditional.
...
struct AstStateAction {
  std::string node_id;     // "hand" (from #hand)
  std::string property;    // "rotation"
  std::string expression;  // "angle * 6"
};
...
struct AstState {
  ...
  std::vector<AstStateAction> actions;  // set #node.prop: ${expr}
  std::unordered_map<std::string, std::string> animation_params; // param -> expression
