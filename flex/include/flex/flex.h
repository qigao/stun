...
#include "flex/fsm.h" // NEW: tinyfsm-based state machine system
#include "flex/runtime_machine.h" // NEW: Runtime state machine system
#include "flex/animation.h" // NEW: Runtime animation system
#include "flex/layout.h" // NEW: Flexbox layout engine
...
public:
  ...
  const std::vector<std::shared_ptr<class RuntimeStateMachine>> &machines() const { return impl_->machines; }
  const std::unordered_map<std::string, std::shared_ptr<class RuntimeAnimation>> &animations() const { return impl_->animations; }
...
public:
  ...
  // -------------------------------------------
  // State Machine Control (Phase 2)  // REMOVED: Old machine system
  // -------------------------------------------

#if 0 // REMOVED: Old machine system - use ComponentFsm per-node instead
