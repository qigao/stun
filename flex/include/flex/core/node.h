 ...
 *
 * Memory Management:
 * - Scene graph APIs primarily pass nodes around as raw pointers.
 * - Arena-backed creation uses raw pointers with ArenaAllocator ownership.
 * - Component/DSL helpers may retain nodes via shared_ptr when ownership must outlive local scope.
 * - Use RawPtr / SharedPtr aliases where ownership semantics matter; Ptr is kept as a compatibility alias.
 */
...
class Node {
public:
    using RawPtr = NodeRawPtr;
    using SharedPtr = SharedNodePtr;
    using Ptr = RawPtr;
    ...
    using LifetimeToken = std::weak_ptr<const void>;

    LifetimeToken lifetime_token() const {
    ...
    void bind_allocation_lifetime(const LifetimeToken& lifetime) {
        lifetime_owner_.reset();
        lifetime_token_ = lifetime;
    }
    ...
    bool flex_basis_auto() const { return !layout_ || layout_->flex_basis_auto; }
    ...
    void set_flex_basis(float b) {
        if (layout_ && layout_->flex_basis == b && layout_->flex_basis_auto == (b <= 0.0f)) return;
        ensure_layout();
        layout_->flex_basis = b;
        layout_->flex_basis_auto = b <= 0.0f;
        mark_dirty(DirtyFlags::Layout);
    }
    void set_flex_basis_auto(bool value) {
        if (layout_ && layout_->flex_basis_auto == value) return;
        ensure_layout();
        layout_->flex_basis_auto = value;
        mark_dirty(DirtyFlags::Layout);
    }
    void set_flex(float grow, float shrink = 1.0f, float basis = 0.0f) {
        const bool basis_auto = basis <= 0.0f;
        if (layout_ && layout_->flex_grow == grow && layout_->flex_shrink == shrink &&
            layout_->flex_basis == basis && layout_->flex_basis_auto == basis_auto) return;
        ensure_layout();
        layout_->flex_grow = grow;
        layout_->flex_shrink = shrink;
        layout_->flex_basis = basis;
        layout_->flex_basis_auto = basis_auto;
        mark_dirty(DirtyFlags::Layout);
    }
    ...
    Vec2 to_local(const Vec2& world_pos) const { return inverse(world_transform()) * world_pos; }
    ...
    RawPtr parent() const { return parent_; }
    virtual bool is_group() const { return false; }
    virtual RawPtr find(const std::string& id) { return nullptr; }
    ...
    void on_focus_event(FocusEventCallback callback) { ensure_events(); events_->on_focus_event = std::move(callback); }
    void on_text_input(TextInputEventCallback callback) { ensure_events(); events_->on_text_input = std::move(callback); }
    void on_composition(CompositionEventCallback callback) { ensure_events(); events_->on_composition = std::move(callback); }
    ...
    void fire_focus_event(FocusEvent& event);
    void fire_text_input(TextInputEvent& event);
    void fire_composition(CompositionEvent& event);
    ...
    const Transform& cached_paint_transform() const { return cached_paint_transform_; }
    bool has_cached_paint_transform() const { return has_cached_paint_transform_; }
    void set_cached_paint_transform(const Transform& transform) {
        cached_paint_transform_ = transform;
        has_cached_paint_transform_ = true;
    }
    void invalidate_cache() {
        cached_paint_ = nullptr;
        has_cached_paint_transform_ = false;
    }
...
protected:
    ...
    // Cached transforms
    Transform local_transform_ = Transform{};
    Transform world_transform_ = Transform{};

    void update_local_transform();
    void update_world_transform();

    // Hierarchy
    RawPtr parent_ = nullptr;
    ...
    Transform cached_paint_transform_ = Transform{};
    bool has_cached_paint_transform_ = false;
};
