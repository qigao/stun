 ...
 *
 * Component subtrees use ComponentNodePtr ownership so rebuilds can replace
 * nodes in the scene graph without leaving dangling raw child pointers.
 */
...
public:
  using SharedPtr = std::shared_ptr<StateComponentBinding>;
  using Ptr = SharedPtr;

  StateComponentBinding(ObservableState::SharedPtr state, const std::string &component_name,
                        const Props &initial_props)
      : state_(state), component_name_(component_name), props_(initial_props),
  ...
  void bind(const std::string &state_key, const std::string &prop_name) {
    // Watch state changes
    auto observer_id = state_->watch(
        state_key, [this, state_key, prop_name](const std::string &key, const StateValue &value) {
          // Update prop value
          std::visit([this, &prop_name](const auto &val) { props_[prop_name] = val; }, value);
...
private:
  ...
  ObservableState::SharedPtr state_;
...
inline StateComponentBinding::SharedPtr
create_bound_component(ObservableState::SharedPtr state, const std::string &component_name,
                       const Props &initial_props,
...
public:
  using SharedPtr = std::shared_ptr<ReactiveGroup>;
  using Ptr = SharedPtr;

  static SharedPtr create(ObservableState::SharedPtr state) {
  ...
  void
  add_bound_component(const std::string &component_name, const Props &initial_props,
                      const std::unordered_map<std::string, std::string> &state_to_prop_bindings,
                      float x = 0, float y = 0) {
    auto binding =
        create_bound_component(state_, component_name, initial_props, state_to_prop_bindings);
  ...
  ObservableState::SharedPtr state() const { return state_; }

private:
  ReactiveGroup() = default;

  ObservableState::SharedPtr state_;
  std::vector<StateComponentBinding::SharedPtr> bindings_;
};
