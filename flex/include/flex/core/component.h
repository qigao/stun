...
// Components return shared-owned node trees because they may be rebuilt,
// retained, and reattached independently from arena-backed scene nodes.
using ComponentNodePtr = SharedNodePtr;
...
public:
    ...
    using BuilderFunc = std::function<ComponentNodePtr(const Props&)>;
    ...
    ComponentNodePtr instantiate(const Props& props = {}) const;
...
public:
    ...
    // Register component
    void register_component(Component::SharedPtr component);
    void register_component(const std::string& name, Component::BuilderFunc builder);

    // Lookup
    Component::SharedPtr get(const std::string& name) const;
...
inline Component::SharedPtr get_component(const std::string& name) {
    return ComponentRegistry::instance().get(name);
}

// Instantiate a component by name
inline ComponentNodePtr create_component_instance(
