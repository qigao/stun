...
#include "flex/core/allocator.h"  // For ArenaAllocator
#include <vector>
#include <memory>
...
public:
    ...
    // Factory (Arena mode)
    static RawPtr create(ArenaAllocator& arena) {
        auto g = arena.create<Group>();
        g->allocator_ = &arena;
        return g;
    }

    // Factory (shared ownership mode)
    ...
    const std::vector<Node::RawPtr>& children() const { return children_; }
    size_t child_count() const { return children_.size(); }

    void add_child(Node::RawPtr child);
    ...
    void add_child(const std::shared_ptr<T>& child) {
        if (!child) return;
        add_child(child.get());
        shared_children_[child.get()] = std::static_pointer_cast<Node>(child);
    }

    void remove_child(Node::RawPtr child);
    void remove_child_at(size_t index);
    void insert_child(Node::RawPtr child, size_t index);
    ...
    Node::RawPtr child_at(size_t index) const;
    Node::RawPtr find_child(const std::string& id) const;
    Node::RawPtr find_child_recursive(const std::string& id) const;

    // Override Node::find() to enable hierarchical search
    Node::RawPtr find(const std::string& id) override { return find_child_recursive(id); }

    // Query by tag
    std::vector<Node::RawPtr> find_by_tag(const std::string& tag) const;
...
private:
    Node::SharedPtr take_owned_child(Node::RawPtr child);

    std::vector<Node::RawPtr> children_;  // children are addressed by raw pointer; ownership may be arena- or shared_ptr-backed
    std::unordered_map<Node::RawPtr, Node::SharedPtr> shared_children_;
