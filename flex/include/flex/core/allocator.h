...
private:
    ...
    template<typename T>
    static auto bind_lifetime(T* object, const std::weak_ptr<const void>& lifetime, int)
        -> decltype(object->bind_allocation_lifetime(lifetime), void()) {
        object->bind_allocation_lifetime(lifetime);
    }

    template<typename T>
    static void bind_lifetime(T*, const std::weak_ptr<const void>&, long) {}
...
public:
    ...
    T* create(Args&&... args) {
        ...
        bind_lifetime(object, lifetime_, 0);
        return object;
    }
