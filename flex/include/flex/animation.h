        ...
        switch (type) {
            ...
            case EasingType::CubicBezier: {
                ...
                float y_t = (1 - param) * (1 - param) * (1 - param) * 0 +
                            ...
                            3 * (1 - param) * param * param * bezier_y2 +
                            param * param * param * 1;
...
inline uint32_t parse_color_string(const std::string& color_str) {
    if (color_str.empty() || color_str[0] != '#') return 0x000000FF;
...
public:
    RuntimeKeyframe(float time, const parser::AstValue& value)
...
public:
    ...
    std::vector<RuntimeKeyframe>& keyframes() { return keyframes_; }  // Non-const version for AnimationManager
...
private:
    ...
    void apply_visual_value(Node* node, const std::string& property, const parser::AstValue& value) const;
    void apply_text_value(Node* node, const std::string& property, const parser::AstValue& value) const;
};
...
inline RuntimeTrack::RuntimeTrack(const std::string& property)
    : property_(property) {}
...
inline void RuntimeTrack::add_keyframe(float time, const parser::AstValue& value) {
    ...
    std::sort(keyframes_.begin(), keyframes_.end(),
        [](const RuntimeKeyframe& a, const RuntimeKeyframe& b) {
            return a.time() < b.time();
        });
}

inline void RuntimeTrack::evaluate(float time, Node* target_node, const EasingFunction& easing) const {
    if (keyframes_.empty() || !target_node) return;
...
inline void RuntimeTrack::apply_value(Node* node, const std::string& property, const parser::AstValue& value) const {
    ...
    if (property[0] == '#') {
        ...
        // Direct property
        if (property == "x" || property == "y" || property == "scale" ||
            ...
            apply_visual_value(node, property, value);
        } else if (property == "content" || property == "color" || property == "text.color" || property == "fontSize") {
            apply_text_value(node, property, value);
        }
    }
}

inline void RuntimeTrack::apply_transform_value(Node* node, const std::string& property, float value) const {
    if (property == "x") {
        node->set_x(value);
    } else if (property == "y") {
        node->set_y(value);
...
inline void RuntimeTrack::apply_visual_value(Node* node, const std::string& property, const parser::AstValue& value) const {
...
inline void RuntimeTrack::apply_text_value(Node* node, const std::string& property, const parser::AstValue& value) const {
...
inline RuntimeAnimation::RuntimeAnimation(const std::string& name)
    : name_(name) {}
