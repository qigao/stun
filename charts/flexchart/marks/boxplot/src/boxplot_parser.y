%name BoxplotChartParser
%token_prefix BOXPLOT_
...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "boxplot: syntax error";
    }
}

start ::= BOXPLOT LBRACE body RBRACE.
start ::= BOXPLOT LBRACE RBRACE.
...
stmt ::= MIN COLON STRING(V).          { ctx->add_simple_encoding("min", V); free(V); }
stmt ::= Q1 COLON STRING(V).           { ctx->add_simple_encoding("q1", V); free(V); }
stmt ::= MEDIAN COLON STRING(V).       { ctx->add_simple_encoding("median", V); free(V); }
stmt ::= Q3 COLON STRING(V).           { ctx->add_simple_encoding("q3", V); free(V); }
stmt ::= MAX COLON STRING(V).          { ctx->add_simple_encoding("max", V); free(V); }
...
enc_stmt ::= MIN COLON STRING(V).     { ctx->add_simple_encoding("min", V); free(V); }
enc_stmt ::= Q1 COLON STRING(V).      { ctx->add_simple_encoding("q1", V); free(V); }
enc_stmt ::= MEDIAN COLON STRING(V).  { ctx->add_simple_encoding("median", V); free(V); }
enc_stmt ::= Q3 COLON STRING(V).      { ctx->add_simple_encoding("q3", V); free(V); }
enc_stmt ::= MAX COLON STRING(V).     { ctx->add_simple_encoding("max", V); free(V); }
