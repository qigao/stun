...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "rule: syntax error";
    }
}

start ::= RULE LBRACE body RBRACE.
start ::= RULE LBRACE RBRACE.
...
stmt ::= X2 COLON STRING(V).           { ctx->add_simple_encoding("x2", V); free(V); }
stmt ::= Y2 COLON STRING(V).           { ctx->add_simple_encoding("y2", V); free(V); }
...
enc_stmt ::= X2 COLON STRING(V).      { ctx->add_simple_encoding("x2", V); free(V); }
enc_stmt ::= Y2 COLON STRING(V).      { ctx->add_simple_encoding("y2", V); free(V); }
