...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "point: syntax error";
    }
}

start ::= POINT LBRACE body RBRACE.
start ::= POINT LBRACE RBRACE.
...
stmt ::= SHAPE COLON IDENT(V).         { ctx->add_style("shape", V); free(V); }
stmt ::= SHAPE COLON STRING(V).        { ctx->add_style("shape", V); free(V); }
...
style_stmt ::= SHAPE COLON IDENT(V).      { ctx->add_style("shape", V); free(V); }
style_stmt ::= SHAPE COLON STRING(V).     { ctx->add_style("shape", V); free(V); }
