...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "text: syntax error";
    }
}

start ::= TEXT_KW LBRACE body RBRACE.
start ::= TEXT_KW LBRACE RBRACE.
...
stmt ::= TEXT COLON STRING(V).         { ctx->add_simple_encoding("text", V); free(V); }
...
stmt ::= FONT_SIZE COLON NUMBER(V).    { ctx->add_style_num("font-size", atof(V)); free(V); }
...
enc_stmt ::= TEXT COLON STRING(V).    { ctx->add_simple_encoding("text", V); free(V); }
...
style_stmt ::= FONT_SIZE COLON NUMBER(V).    { ctx->add_style_num("font-size", atof(V)); free(V); }
