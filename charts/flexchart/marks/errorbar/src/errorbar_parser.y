%name ErrorbarChartParser
%token_prefix ERRORBAR_
...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "errorbar: syntax error";
    }
}

start ::= ERRORBAR LBRACE body RBRACE.
start ::= ERRORBAR LBRACE RBRACE.
...
stmt ::= Y_ERROR COLON STRING(V).      { ctx->add_simple_encoding("y-error", V); free(V); }
stmt ::= Y_ERROR_MIN COLON STRING(V).  { ctx->add_simple_encoding("y-error-min", V); free(V); }
stmt ::= Y_ERROR_MAX COLON STRING(V).  { ctx->add_simple_encoding("y-error-max", V); free(V); }
...
stmt ::= CAP_SIZE COLON NUMBER(V).     { ctx->add_style_num("cap-size", atof(V)); free(V); }
...
enc_stmt ::= Y_ERROR COLON STRING(V).      { ctx->add_simple_encoding("y-error", V); free(V); }
enc_stmt ::= Y_ERROR_MIN COLON STRING(V).  { ctx->add_simple_encoding("y-error-min", V); free(V); }
enc_stmt ::= Y_ERROR_MAX COLON STRING(V).  { ctx->add_simple_encoding("y-error-max", V); free(V); }
...
style_stmt ::= CAP_SIZE COLON NUMBER(V).  { ctx->add_style_num("cap-size", atof(V)); free(V); }
stmt ::= data_block.
