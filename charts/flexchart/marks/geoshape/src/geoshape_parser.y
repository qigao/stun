%name GeoshapeChartParser
%token_prefix GEOSHAPE_
...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "geoshape: syntax error";
    }
}

start ::= GEOSHAPE LBRACE body RBRACE.
start ::= GEOSHAPE LBRACE RBRACE.
...
stmt ::= SHAPE COLON STRING(V).        { ctx->add_simple_encoding("shape", V); free(V); }
stmt ::= COLOR COLON STRING(V).        { ctx->add_simple_encoding("color", V); free(V); }

// --- Range encoding: shape: [start, end, step] ---
stmt ::= SHAPE COLON LBRACKET NUMBER(A) COMMA NUMBER(B) COMMA NUMBER(C) RBRACKET. {
    ctx->add_range("shape", atof(A), atof(B), atof(C)); free(A); free(B); free(C);
}
stmt ::= COLOR COLON LBRACKET NUMBER(A) COMMA NUMBER(B) COMMA NUMBER(C) RBRACKET. {
    ctx->add_range("color", atof(A), atof(B), atof(C)); free(A); free(B); free(C);
}
...
stmt ::= PROJECTION COLON IDENT(V).    { ctx->add_style("projection", V); free(V); }
stmt ::= PROJECTION COLON STRING(V).   { ctx->add_style("projection", V); free(V); }
...
enc_stmt ::= SHAPE COLON STRING(V).   { ctx->add_simple_encoding("shape", V); free(V); }
...
style_stmt ::= PROJECTION COLON IDENT(V). { ctx->add_style("projection", V); free(V); }
style_stmt ::= PROJECTION COLON STRING(V). { ctx->add_style("projection", V); free(V); }
