...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "rect: syntax error";
    }
}

start ::= RECT LBRACE body RBRACE.
start ::= RECT LBRACE RBRACE.
...
stmt ::= CELL_WIDTH COLON NUMBER(V).   { ctx->add_style_num("cell-width", atof(V)); free(V); }
stmt ::= CELL_HEIGHT COLON NUMBER(V).  { ctx->add_style_num("cell-height", atof(V)); free(V); }
...
style_stmt ::= CELL_WIDTH COLON NUMBER(V).   { ctx->add_style_num("cell-width", atof(V)); free(V); }
style_stmt ::= CELL_HEIGHT COLON NUMBER(V).  { ctx->add_style_num("cell-height", atof(V)); free(V); }
