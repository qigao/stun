...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "image: syntax error";
    }
}

start ::= IMAGE LBRACE body RBRACE.
start ::= IMAGE LBRACE RBRACE.
...
stmt ::= URL COLON STRING(V).          { ctx->add_simple_encoding("url", V); free(V); }
...
stmt ::= IMG_WIDTH COLON NUMBER(V).    { ctx->add_style_num("img-width", atof(V)); free(V); }
stmt ::= IMG_HEIGHT COLON NUMBER(V).   { ctx->add_style_num("img-height", atof(V)); free(V); }
...
enc_stmt ::= URL COLON STRING(V).     { ctx->add_simple_encoding("url", V); free(V); }
...
style_stmt ::= IMG_WIDTH COLON NUMBER(V).  { ctx->add_style_num("img-width", atof(V)); free(V); }
style_stmt ::= IMG_HEIGHT COLON NUMBER(V). { ctx->add_style_num("img-height", atof(V)); free(V); }
