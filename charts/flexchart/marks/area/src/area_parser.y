...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "area: syntax error";
    }
}

start ::= AREA LBRACE body RBRACE.
start ::= AREA LBRACE RBRACE.
...
stmt ::= OPACITY COLON NUMBER(V).      { ctx->add_style_num("opacity", atof(V)); free(V); }
...
style_stmt ::= OPACITY COLON NUMBER(V).      { ctx->add_style_num("opacity", atof(V)); free(V); }
