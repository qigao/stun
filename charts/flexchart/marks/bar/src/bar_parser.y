...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "bar: syntax error";
    }
}

start ::= BAR LBRACE body RBRACE.
start ::= BAR LBRACE RBRACE.
...
stmt ::= STACK COLON TRUE.             { ctx->add_style_bool("stack", 1); }
stmt ::= STACK COLON FALSE.            { ctx->add_style_bool("stack", 0); }
stmt ::= STACK COLON IDENT(V).         { ctx->add_style_bool("stack", strcmp(V,"true")==0); free(V); }
stmt ::= CORNER_RADIUS COLON NUMBER(V). { ctx->add_style_num("corner-radius", atof(V)); free(V); }
stmt ::= ORIENTATION COLON IDENT(V).   { ctx->add_style("orientation", V); free(V); }
...
style_stmt ::= CORNER_RADIUS COLON NUMBER(V). { ctx->add_style_num("corner-radius", atof(V)); free(V); }
style_stmt ::= STACK COLON TRUE.              { ctx->add_style_bool("stack", 1); }
style_stmt ::= STACK COLON FALSE.             { ctx->add_style_bool("stack", 0); }
style_stmt ::= STACK COLON IDENT(V).          { ctx->add_style_bool("stack", strcmp(V,"true")==0); free(V); }
style_stmt ::= ORIENTATION COLON IDENT(V).    { ctx->add_style("orientation", V); free(V); }
