...
extern "C" void flexchart_pie_force_link(void) {}
...
void PieMarkRenderer::render(const std::shared_ptr<AstMark>& mark, const std::vector<Record>& records, MarkRenderContext& ctx) {
    ...
    for (const auto& enc : mark->encodings) {
        if (enc->channel == "theta") y_field = enc->field;
        else if (enc->channel == "color") x_field = enc->field;
    }
    ...
    float start_angle = -FLEX_PI / 2.0f;
...
REGISTER_MARK_RENDERER("pie", flex::chart::PieMarkRenderer)
