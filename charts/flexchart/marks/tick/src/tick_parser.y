...
%syntax_error {
    ...
    if (ctx->error_message.empty()) {
        ctx->error_message = "tick: syntax error";
    }
}

start ::= TICK LBRACE body RBRACE.
start ::= TICK LBRACE RBRACE.
...
stmt ::= SIZE COLON NUMBER(V).         { ctx->add_style_num("size", atof(V)); free(V); }
stmt ::= ORIENT COLON IDENT(V).        { ctx->add_style("orient", V); free(V); }
...
style_stmt ::= SIZE COLON NUMBER(V).      { ctx->add_style_num("size", atof(V)); free(V); }
style_stmt ::= ORIENT COLON IDENT(V).     { ctx->add_style("orient", V); free(V); }
