...
void RuleMarkRenderer::render(const std::shared_ptr<AstMark>& mark, const std::vector<Record>& records,
    ...
    for (const auto& rec : records) {
        ...
        // Horizontal rule (threshold line)
        ...
        // Vertical rule
            ...
            y1 = 0;
            y2 = ctx.estimated_plot_h;
        }
        // Custom line segment
        else {
            if (!x_field.empty()) {
                std::string xv = get_string_val(rec.get(x_field));
                x1 = get_x_pos(xv, ctx);
            }
            if (!x2_field.empty()) {
                std::string xv = get_string_val(rec.get(x2_field));
                x2 = get_x_pos(xv, ctx);
            } else {
                x2 = x1;
            }
            if (!y_field.empty()) {
                float yv = (float)get_double_val(rec.get(y_field));
                y1 = ctx.estimated_plot_h - (yv * ctx.y_scale);
            }
            if (!y2_field.empty()) {
                float yv = (float)get_double_val(rec.get(y2_field));
                y2 = ctx.estimated_plot_h - (yv * ctx.y_scale);
            } else {
                y2 = y1;
            }
        }
