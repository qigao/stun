...
void RadarMarkRenderer::render(const std::shared_ptr<AstMark>& mark, const std::vector<Record>& records, MarkRenderContext& ctx) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        // Use a local scale for radar (relative to r_max)
        float dist = (float)((val / 100.0) * r_max); // Normalize to 100 max
        ...
        // Category labels around the edge
