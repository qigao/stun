...
3. **`cssbox_svg_path_new.cpp`** - Parser implementation
...
```bash
./build/cssbox/tests/test_svg_path_parser
```
...
// Parse a simple path
auto commands = cssbox::SVGPathParser::parse("M 10 20 L 30 40 Z");
...
This implementation follows the same license as the parent project (cssbox).
