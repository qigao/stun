...
将 cssbox 从传统 `new`/`delete` 迁移到高性能内存池。
...
### Step 1: 修改 cssboxRenderer

**文件**: `cssbox/include/cssbox_internal.h`
...
public:
    ...
    // Memory management
    cssbox::MemoryArena& arena() { return arena_; }
    cssbox::ObjectPool<TransitionState>& transition_pool() { return transition_pool_; }
    cssbox::ObjectPool<AnimationState>& animation_pool() { return animation_pool_; }
...
**文件**: `cssbox/src/cssbox.cpp`

**Before** (Line 871):
```cpp
auto get_transition_state = [](cssboxElement* element) -> TransitionState* {
...
**Before** (Line 1024):
```cpp
auto get_animation_state = [](cssboxElement* elem) -> AnimationState* {
...
**文件**: `cssbox/src/cssbox_painter.cpp`
...
```cpp
// 使用 arena 临时分配
cssbox::MemoryScope scope(renderer_->arena());
...
**文件**: `cssbox/src/cssbox_svg_path.cpp`
...
```cpp
// 添加 arena 参数
std::vector<SVGPathCommand, cssbox::ArenaAllocator<SVGPathCommand>> 
parse(const std::string& d, cssbox::MemoryArena& arena) {
    std::vector<SVGPathCommand, cssbox::ArenaAllocator<SVGPathCommand>> 
        commands(cssbox::ArenaAllocator<SVGPathCommand>(arena));  // ✅
    // ... 解析逻辑
    return commands;
}
```
...
**文件**: `cssbox/src/cssbox_svg_xml.cpp`
...
**文件**: `cssbox/tests/test_memory_pool.cpp`
...
void test_object_pool() {
    cssbox::MemoryArena arena(1024);
    cssbox::ObjectPool<int> pool(arena);
...
**文件**: `cssbox/tests/benchmark_memory.cpp`
...
void benchmark_pool(int iterations) {
    cssbox::MemoryArena arena(1024 * 1024);
    cssbox::ObjectPool<TestObject> pool(arena);
...
```cpp
// ❌ 错误：返回 arena 分配的指针
const char* get_class_name(cssbox::MemoryArena& arena) {
...
// ✅ 正确：每线程独立 arena
thread_local cssbox::MemoryArena arena(1024 * 1024);
```
