...
// This file is included directly into cssbox_painter.cpp
// All necessary headers are already included there

// Helper for UTF-8 decoding
static int cssbox_utf8_next(const char** str) {
...
void cssboxPainter::paint_text_path(const cssboxElement* element, const cssboxBox& box) {
    ...
    // 2. Find path element
    cssboxElement* path_element = cssboxGetElement(renderer_, path_id.c_str());
    ...
    if (offset_it != element->attributes.end()) {
        start_offset = cssbox_utils::parse_length(offset_it->second, total_length);
    }
    ...
    while (iter < end) {
        ...
        int codepoint = cssbox_utf8_next(&next);
        ...
        if (sample_dist >= 0 && sample_dist <= total_length) {
            cssbox::PathSample sample = cssbox::interpolate_path_position(samples, sample_dist);
