...
TEST_CASE("SVG Path Parser - MoveTo and LineTo", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M 10 20 L 30 40");
    REQUIRE(cmds.size() == 2);
...
TEST_CASE("SVG Path Parser - Cubic Bezier", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M 0 0 C 10 10 20 20 30 30");
    REQUIRE(cmds.size() == 2);
...
TEST_CASE("SVG Path Parser - Relative Commands", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("m 10 20 l 5 5");
    REQUIRE(cmds.size() == 2);
    REQUIRE(cmds[0].type == 'M');
...
TEST_CASE("SVG Path Parser - Implicit Commands", "[svg][path]") {
    ...
    auto cmds = cssbox::SVGPathParser::parse("M 10 20 30 40");
    REQUIRE(cmds.size() == 2);
    REQUIRE(cmds[0].type == 'M');
    REQUIRE(cmds[1].type == 'L');
    REQUIRE(cmds[1].params[0] == 30.0f);
...
TEST_CASE("SVG Path Parser - Complex Path", "[svg][path]") {
    ...
    auto cmds = cssbox::SVGPathParser::parse("M 990 275 Q 1010 250 1030 275 T 1070 275");
    REQUIRE(cmds.size() == 3);
    REQUIRE(cmds[0].type == 'M');
...
TEST_CASE("SVG Path Parser - Arc Command", "[svg][path]") {
    ...
    auto cmds = cssbox::SVGPathParser::parse("M 10 10 A 30 50 45 0 1 100 200");
    REQUIRE(cmds.size() == 2);
...
TEST_CASE("SVG Path Parser - Negative Numbers", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M -10 -20 L -30.5 -40.75");
    REQUIRE(cmds.size() == 2);
    REQUIRE(cmds[0].params[0] == -10.0f);
    REQUIRE(cmds[0].params[1] == -20.0f);
...
TEST_CASE("SVG Path Parser - Scientific Notation", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M 1e2 2.5e-1 L 3E+2 4.0E0");
    REQUIRE(cmds.size() == 2);
...
TEST_CASE("SVG Path Parser - Mixed Separators", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M10,20L30 40");
    REQUIRE(cmds.size() == 2);
    REQUIRE(cmds[0].params[0] == 10.0f);
    REQUIRE(cmds[1].params[0] == 30.0f);
}
...
TEST_CASE("SVG Path Parser - Compressed Path", "[svg][path]") {
    ...
    auto cmds = cssbox::SVGPathParser::parse("M10 20L30 40z");
    REQUIRE(cmds.size() == 3);
    REQUIRE(cmds[0].type == 'M');
    REQUIRE(cmds[1].type == 'L');
...
TEST_CASE("SVG Path Parser - Multiple Implicit Commands", "[svg][path]") {
    ...
    auto cmds = cssbox::SVGPathParser::parse("M 0 0 10 10 20 20");
    REQUIRE(cmds.size() == 3);
    REQUIRE(cmds[0].type == 'M');
    REQUIRE(cmds[1].type == 'L');
    REQUIRE(cmds[1].params[0] == 10.0f);
...
TEST_CASE("SVG Path Parser - Horizontal and Vertical Lines", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M 10 10 H 50 V 100");
    REQUIRE(cmds.size() == 3);
...
TEST_CASE("SVG Path Parser - Smooth Curves", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("M 0 0 C 10 10 20 20 30 30 S 50 50 60 60");
    REQUIRE(cmds.size() == 3);
    REQUIRE(cmds[1].type == 'C');
...
TEST_CASE("SVG Path Parser - Empty Path", "[svg][path]") {
    auto cmds = cssbox::SVGPathParser::parse("");
    REQUIRE(cmds.empty());
}
