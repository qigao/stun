...
This directory contains comprehensive tests for the cssbox module.

## Test Types

### 1. Unit Tests (Automated)

**`test_cssbox_comprehensive.cpp`**
...
- **Run with**: `ctest -R test_cssbox_comprehensive`

**`test_cssbox_advanced.cpp`**
...
- **Run with**: `ctest -R test_cssbox_advanced`
...
```bash
# Build all tests
cmake --build build --target test_cssbox_comprehensive
cmake --build build --target test_cssbox_advanced
...
ctest -R test_cssbox_comprehensive --verbose
...
Edit `test_cssbox_comprehensive.cpp` or `test_cssbox_advanced.cpp`:
...
tests.push_back({
    ...
    [](cssboxRenderer* r, float x, float y, float w, float h) {
        cssboxElement* elem = cssboxCreateElement(r, "id", "type");
        // Setup element...
    }
});
```
...
The unit tests (`test_cssbox_comprehensive` and `test_cssbox_advanced`) can run in CI/CD pipelines without a display since they use nullptr for NVGcontext.
...
- name: Run NanoVG CSS Tests
  run: |
    cd build
    ctest -R cssbox --output-on-failure
```
