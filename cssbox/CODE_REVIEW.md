...
| 文件 | 行数 | 复杂度 | 状态 |
|------|------|--------|------|
| cssbox_painter.cpp | 2680 | 高 | ⚠️ 需要拆分 |
| cssbox_grid.cpp | 1937 | 高 | ⚠️ 需要重构 |
| lexbor_css_parser.cpp | 1922 | 中 | ✅ 可接受 |
| cssbox_quadtree.cpp | 1593 | 中 | ✅ 可接受 |
| cssbox.cpp | 1411 | 中 | ✅ 可接受 |
| cssbox_utils.cpp | 981 | 低 | ✅ 良好 |
...
**位置**: `cssbox_painter.cpp:2095-2350`
- `paint_svg_path()`: ~250 行
- 包含复杂的椭圆弧计算

**修复方案**: 拆分为子函数
```cpp
void paint_svg_path(const cssboxElement* element, const cssboxBox& box) {
...
**关键 TODO**:
1. `cssbox_utils.cpp:364` - 缺少 cubic-bezier 实现
2. `cssbox_quadtree.cpp:1114` - 缺少 auto_cols_str 解析
3. `cssbox_painter.cpp:918` - custom_paint 签名需要更新
...
```
cssbox.cpp          - 核心 API 和元素管理
cssbox_painter.cpp  - 渲染逻辑
cssbox_quadtree.cpp - 布局引擎
...
### 必须修复（阻塞发布）
- [ ] 修复 `malloc` 内存泄漏 (`cssbox_svg_xml.cpp:397`)
