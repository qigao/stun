...
public:
    ...
    static void window_refresh_callback(GLFWwindow* win) {
        auto* app = static_cast<LayoutsDemo*>(glfwGetWindowUserPointer(win));
        if (app) {
            app->render();
            glfwSwapBuffers(win);
        }
...
private:
    ...
    void setup_scene() {
        ...
        // === Create Flexbox Row ===
        auto* flex_row = cssboxCreateElement(renderer, "flex-row", "div");
        for (int i = 0; i < 5; i++) {
            auto* item = cssboxCreateElement(renderer, "", "div");
            cssboxAddClass(item, "flex-item");
            cssboxAppendChild(renderer, flex_row, item);
        }

        // === Create Flexbox Column ===
        auto* flex_col = cssboxCreateElement(renderer, "flex-column", "div");
        for (int i = 0; i < 3; i++) {
            auto* item = cssboxCreateElement(renderer, "", "div");
            cssboxAddClass(item, "flex-item");
            cssboxAppendChild(renderer, flex_col, item);
        }

        // === Create Grid Layout ===
        auto* grid = cssboxCreateElement(renderer, "grid-container", "div");
        for (int i = 0; i < 6; i++) {
            auto* item = cssboxCreateElement(renderer, "", "div");
            cssboxAddClass(item, "grid-item");
            cssboxAppendChild(renderer, grid, item);
        }

        // === Create Grid Areas ===
        auto* grid_areas = cssboxCreateElement(renderer, "grid-areas", "div");
        cssboxAppendChild(renderer, grid_areas, cssboxCreateElement(renderer, "header", "div"));
        cssboxAppendChild(renderer, grid_areas, cssboxCreateElement(renderer, "sidebar", "div"));
        cssboxAppendChild(renderer, grid_areas, cssboxCreateElement(renderer, "main", "div"));
        cssboxAppendChild(renderer, grid_areas, cssboxCreateElement(renderer, "footer", "div"));

        // === Create Positioning Demo ===
        auto* pos_container = cssboxCreateElement(renderer, "position-container", "div");
        cssboxAppendChild(renderer, pos_container, cssboxCreateElement(renderer, "pos-relative", "div"));
        cssboxAppendChild(renderer, pos_container, cssboxCreateElement(renderer, "pos-absolute", "div"));
