...
inline VerticalAlign parse_vertical_align(std::string_view str) {
    auto lower = to_lowercase(trim(str));
    if (lower == "top") return VerticalAlign::TOP;
    if (lower == "middle") return VerticalAlign::MIDDLE;
    if (lower == "bottom") return VerticalAlign::BOTTOM;
    if (lower == "baseline") return VerticalAlign::BASELINE;
    return VerticalAlign::TOP;  // Default
}
